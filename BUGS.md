...
⛔ **And the other 24 of the 31 in the six read documents were RENDERED the same day, which moves the
population a third of the way to zero**: `Jones et al_2010` p7 loses the paper's **estimating
equation** at `inkOut` **0.0137** and its p2 loses a word at **0.0008**, against a previous lowest
measured loser of 0.0353 — so "the pages near the bar" was never the population, and **27 of the 73
print `0.0000`** and, measured over all four sub-steps, **25 of them** cannot be priced through the
override seam at all (no legal `INKBAR` is below 0; two of the 27 flip anyway).
`Broadhead - 1994`'s two lowest pages lose nothing, so its window holds at [0.04415, 0.04495] while
`Jones`'s is at most 0.0137 — **two documents that each order their own pages, with disjoint windows**,
and 94.7% of the 13 priced pages' +1,566,676 B buys nothing a reader would miss. ⛔ **20 more were
rendered the same day — the twenty highest-`inkOut` of what was left — and 8 of those 20 lose
content**: three lines of 1958 magazine prose over two pages, a line and a word of 1930s
typescript, a word of 1913 body text, a page folio, a signature, a pencilled annotation and a
...
*usable* bar reaches — read the ⚠️ below `#### The other 24…` for why "prints 0.0000" is the strongest
statement the seam supports, and `#### The last 21…` for why neither reading of it leaves a usable bar.
⚠️ Three figures the entry was carrying are corrected by that render:
**25 of the 73**, not 27, cannot be priced through the override seam (two pages print `0.0000` and flip
anyway); the page with the **highest** out-of-stencil ink fraction of the 21 — 11.65x its own
...
10** and none of the six accepted groups is type: halftone dots, photograph grain inside a plate whose
real caption Vision *did* recognise (12 groups, none on the caption), a pen ornament's feather strokes
(17 and 11), and 10,438 px of page-wide-Otsu speckle in a featureless grey endpaper straddling the
threshold by ±3 levels. ⛔ **But the sharper half is the other direction: it reads 0 on two pages whose
...
(`glyphN` — so "too little type to calibrate on" is **not** the mechanism — plus `txtShare` again,
absolute `linePx`, `topLine` width). ✅ **The bound holds for ONE of the two seams**: all ten are
`barVerdict=picture`, so `inkOut < 0.045 AND lineN == 0` never consults the term on them and the byte
cost is zero — derived from the sweep, not measured, and on a **1.10x** margin (0.0493 against 0.045).
⛔ **Under `textRegionMask`, which is the seam question 3's own sentence names, there is no bound**: it
runs unconditionally on every layered page (`Flattener.swift:2696`), so those six pages would put
...
lines that are in the text layer, read at 1:1 — so the candidate fix was subtracting a *dilated*
`region` before grouping. ⛔ **THAT FIX WAS TRIED THE SAME DAY OVER THE SAME 73 AND IT SPLITS IN TWO** —
`SHAPETERM-RIM-2026-08-21.tsv`, `#### The rim fix, MEASURED`. Type-losers firing / non-losers firing:
r=0 **12/12, 3/51**; r=1 **12/12, 2/51**; r=2 **11/12, 1/51**; r=3 **9/12, 1/51**. ⛔ **As a REPLACEMENT
for the rule it is refused**: r=1 is the only radius that keeps every real loss, and it clears **two**
rims of the three while **adding one of its own** — on `Xin Qu et al_2018` p28 the rim of a recognised
`469.` is three accepted components at r=0, one short of `lineMinimumMembers`, and a 1-px collar splits
...
`#### The eight near-misses, RENDERED`, then the other 24 pages
of the six documents C26's sub-step 4 had read, in `#### The other 24 in the six read documents, RENDERED`,
then the twenty highest-`inkOut` of what was left, in
`#### The twenty highest-inkOut of the remaining 41, RENDERED`, then the last 21, in
`#### The last 21 of the 73, RENDERED`. 8 + 24 + 20 + 21 = 73, and **16 of the 73 lose content**;
everything before those four sections is the mechanism, the evidence C26 already paid for, and the
first measurement. ⛔ Those sections correct this entry's own instrument line — the one in question 1 of
...
**Question 3 has two halves and both are measured**: the shape term separates on 13 labelled pages
(`#### A shape term, MEASURED over 13 labelled pages`) and then **errs in both directions on 10 picture
...
reads does not order the loss**. ⛔ **And measured the same day over 24 more pages, it happens a third
of the way to zero**: `Jones et al_2010` p7 loses the paper's **estimating equation** at `inkOut`
**0.0137** and its p2 loses a word at **0.0008**, so the population at risk is not "the pages near the
bar" — 27 of the 73 print `0.0000`, one page at 0.0008 is already losing type, and sub-step 4
found one of those `0.0000` pages losing a pencilled annotation. ⛔ **And
measured the same day over the twenty highest-`inkOut` pages left, the loss is a MIDDLE band**: 8 of
20 lose content, their `inkOut` runs [0.0051, 0.0165], and of the twelve that lose nothing **four sit
above that band and six below it** — so the signal does not merely fail to separate the two sets, it
is not monotone in the direction `pageIsAllText()` assumes, and the four pages with the most ink
outside the recognised words are the first four any bar protects and lose nothing at all.
⛔ **And the last 21 of the 73, rendered the same day, close the population and refute the bar from the
...
See `#### The eight near-misses, RENDERED` below; and **24 more were rendered the same day** —
`#### The other 24 in the six read documents, RENDERED` — of which **2 lose content** (an estimating
equation, a word), **6 more lose something** (a hand-drawn pen line and five footnote rules) and 16
lose nothing a reader sees; and **20 more the same day** —
`#### The twenty highest-inkOut of the remaining 41, RENDERED` — of which **8 lose content** (a line
and a word of typescript, three lines of magazine prose over two pages, a word of body text, a page
folio, a signature, a pencilled annotation and a hand-inked margin number); ✅ **and the last 21 the
same day** — `#### The last 21 of the 73, RENDERED` — **of which 2 lose content, so all 73 are read
and 16 of the 73 lose content.** (This sentence read "whether any of the 73 is
losing content is **not known** — none has been rendered", then "65 remain unrendered", then "41",
until then.)
...
✅ **all 31 of which have now been read** (7 in sub-step 1, the other 24 in the sub-step below;
2026-08-20 — this sentence ended "none of them looked at", then "7 have now been looked at and 24 have
not", until then). What it bought: **4 of the 31 lose type** — `Broadhead - 1994` p3's two lines,
`Jones et al_2010` p5's two lines, `Jones` p7's estimating equation and `Jones` p2's single word — and
**2 more lose hand-drawn marks**, `Atkinson_1939` p3's signature outright and `Riesman - 1954` p20's
pen line to a smear. The other 25 lose page edges, gutter, thumbs, a footnote rule or nothing at all.
**41 pages in 15 documents remain, and none of those 15 has been looked at at all**, so the "same scan,
same recogniser" argument is spent.
...
⛔ **A third such page was measured 2026-08-20**: `1976 - Regis McKenna Papers` p4, **+350,632 B at
7.89x** — the steepest ratio in this campaign — whose out-of-stencil ink is platen strips and a staple
shadow and nothing else. ⚠️ **But the sub-step that found it also weakened this section's general
claim, and that is recorded there rather than here**: over its twenty pages a bar set to protect every
loser puts only **37.6%** of its cost on pages that lose nothing, against 57.4% (sub-step 1) and 56.6%
(sub-step 2). How much a bar overpays is a property of the pages sampled, not a constant.
...
at 0.0193 and p2 at 0.0140 are sub-bar and still unrendered) and the shipped 0.045 sits above that window — missing p3 by **at most 0.00005**. ⛔ **Superseded 2026-08-20: p4 and p2 WERE rendered later the same day, they lose nothing, and the window is unchanged — see `#### The other 24 in the six read documents, RENDERED`, which also records that `Riesman - 1954` has no window at all.** ⛔ **But nothing of the kind is
...
`Jones et al_2010` has p2 (0.0008), p3 (0.0000), p5, p7 (0.0137) and p9 (0.0000). ⛔ **Superseded 2026-08-20**: four of those five were rendered later the same day, so `Jones`,
`Atkinson_1939` and `_1973_Committee Against Racism_` all have windows now and the count is 41 rather
than 65 — the next section carries them, and the sentence below ("open questions, not results") was
true only for the few hours between the two sub-steps. What *is* measured for them is only that 0.045 is too high for each.
...
that 31 remain** and are still the cheapest place to continue. ⛔ **Those 24 were rendered on
2026-08-20 — the next section — and then 20 more, and then the last 21, so this paragraph is
superseded three times over: **all 73 are read, none remain, and the six documents are exhausted.**

#### The other 24 in the six read documents, RENDERED — the loss reaches a THIRD of the bar, and quantity does not order it either — MEASURED 2026-08-20

**The population.** The 24 pages of the 73 that sit in the six documents C26 sub-step 4 rendered and
that sub-step 1 did not reach: `Xin Qu et al_2018` p3/p5/p8/p10/p13/p23/p25/p28,
`Riesman - 1954` p6/p10/p12/p14/p20/p24, `Jones et al_2010` p2/p3/p7/p9,
`_1973_Committee Against Racism_` p1/p2/p3, `Broadhead - 1994` p2/p4, `Atkinson_1939` p1. Six
invocations, `rc=0` on every one, `INKDUMP` **7 files per page with none missing on all 24**, and
`inkOut`, `layered` and `extent` reproduce `INKBAR-2026-08-19.tsv` **digit for digit on all 24 rows**
(checked by a script over the committed file, not by eye).

⛔ **FIRST, A BOUND ON THE OVERRIDE SEAM ITSELF, AND IT IS ARITHMETIC RATHER THAN A BUG: 11 of these
24 CANNOT BE PRICED THROUGH IT, AND NEITHER CAN 27 OF THE 73.** `pageIsAllText()` compares
`inkOutsideText(…) < bar` with a strict `<`, so un-shrinking a page needs a bar **at or below** its own
`inkOut`; `Tools/score-text-route.swift` refuses an `INKBAR` outside `(0, 1)`, for the good reason its
own header gives. A page whose `inkOut` is **0** therefore has no legal bar, and 11 of these 24 read
`barDelta` `same` at `INKBAR=0.00001` — `Atkinson_1939` p1, `_1973_Committee Against Racism_` p1/p2/p3,
`Jones` p3/p9, `Riesman - 1954` p6/p10/p14, `Xin Qu` p3/p5. The sweep prints four places and **27 of
the 73 print `0.0000`**, so this is not a corner: it is a third of the population. ⚠️ Stated exactly:
**no legal bar exists only for a page whose `inkOut` is exactly 0**; what is *measured* is that 11 of
these 24 do not flip at 1e-5 and 27 of the 73 print `0.0000`. A page at, say, 3.3e-6 is positive,
priceable at 1e-9, and indistinguishable from 0 at both four places and 1e-5, and it is the same
false-negative shape sub-step 1 corrected (two byte-identical backgrounds read as "loses nothing")
arriving from the other side. ⚠️ Printing `0.0000` is *not* the test — `Riesman - 1954` p12 prints
`0.0000` and **did** flip, so its true value is in [0.00001, 0.00005). The test is `barDelta`.
⚠️ And the summary line cannot show which tiny bar was used: it formats the bar with `%.4f`, so
`INKBAR=0.00001` prints as `INKBAR 0.0000 against the shipped 0.0450`.

**So this sub-step used a second instrument, and it needs no override at all.** From the dump's own
`-source.png` (the grey render) and `-stencil.png`, the marks that survive **only** in the background
are `ink AND NOT stencil`, where ink is the same page-wide Otsu the guard uses:

```sh
magick "$s-source.png" -auto-threshold OTSU -negate ink.png
magick "$s-stencil.png" -morphology Erode Disk:3 notsten.png   # dilate the stencil's INK by 3 px
magick ink.png notsten.png -compose Multiply -composite out.png # 0/255 AND; Multiply is commutative
```

⚠️ **Both halves of that recipe are there because the first version was wrong, measured.** (a) Without
the erosion the map is dominated by an artifact: `inkOutsideText` thresholds with a page-wide Otsu
while the stencil comes from an adaptive Sauvola mask, so every stencilled glyph contributes a
one-or-two-pixel rim of "ink not in the stencil" — 34,158 px on `Atkinson_1939` p1, **9.1% of its
interior ink on a page whose `inkOut` is 0.0000**. Eroding by 3 px takes that page to **15 px**.
⚠️ **The rim is not a constant multiple, and this sentence claimed "3-5x of `inkOut`" until the review
of this diff measured all twelve positive-`inkOut` pages**: the r=0 fraction over `inkOut` runs
**0.97x to 19.4x** — ~1.0x on the eight `Jones`/`Xin Qu` pages (where the rim is negligible:
`Jones` p7 goes 2,248 → 2,208 px, `Xin Qu` p10 151 → 151), 3.3x/3.6x on the two `Broadhead` pages and
8.0x/19.4x on `Riesman` p20/p24. `Atkinson_1939` p1 is the worst case, not the typical one. (b) The first script used
`-compose Minus`, which computes `dst - src` and so measured the *stencil* minus the ink; it was caught
by a dilation sweep in which the counts **rose** with r, and every number it produced is void.

✅ **The locator is validated against a positive and a negative control with verdicts already settled
by eye.** Re-rendered `Broadhead - 1994` p3 and p10 at `INKBAR=0.02` (`inkOut` 0.0450 / 0.0441 and
`layeredAtBar` 244,731 / 248,701 B, reproducing sub-step 1's 4.35x / 4.21x): on **p3** the map holds
exactly the two lines sub-step 1 read by eye — *"alizing a cultural order at that moment passing from
life and of fab- / ricating, in the literary realm, a mentally possessible version of a"* — and on
**p10** it holds page edges, the gutter and the scanner's thumbs and no type at all. It does not
merely agree with the earlier verdicts; it **names the words**.

⛔ **AND THE CONTROL PAIR KILLS A QUANTITY TERM AS SHARPLY AS SUB-STEP 1 KILLED A BAR.** p3, which
loses two lines, holds **15,431 px** of out-of-stencil interior ink (4.48% of its interior ink); p10,
which loses nothing, holds **15,727 px** (4.48%) — the non-loser is *higher* — and both are flat across
`r` = 2 / 3 / 4 (15,495 / 15,431 / 15,377 and 15,750 / 15,727 / 15,711), because what dominates both is
the gutter shadow, a solid region no erosion touches. So "how much ink is outside the stencil" orders
the loss no better than "what fraction of the page's ink is outside the words". Question 3's term has
to be about *what the marks are*, and this is the second measured refusal of a scalar.

**The verdicts.** Each mark located by the map, each verdict settled by a 1:1 three-row crop
(source / background at the bar / background as shipped), and the three that carry content
cross-checked against the **stencil's own ink fraction** in the rect:

| `inkOut` | page | out-of-stencil interior ink | what it is | verdict |
|---|---|---|---|---|
| 0.0193 | `Broadhead - 1994` p4 | 6,912 px | the scanner's thumbs, the gutter, page edges | nothing a reader sees |
| 0.0140 | `Broadhead - 1994` p2 | 4,958 px | thumbs, gutter, page edges | nothing a reader sees |
| **0.0137** | **`Jones et al_2010` p7** | **2,208 px** | **the two displayed lines of the paper's estimating equation (2)** — `y_it = β_y_1 y_{i,t-1} + β_k k_it + β_k_1 k_{i,t-1} + β_l l_it + β_l_1 l_{i,t-1}` and `+ β_op op_it + β_op_1 op_{i,t-1} + γ_t + η_i + ε_it` — **and the parenthetical `(p-value 0.12).`** after "Hansen J statistic supported"; the running-head rule goes with them | **LOSES MATHEMATICS** |
| 0.0022 | `Riesman - 1954` p20 | 1,304 px | a **hand-drawn pen line** down the margin of the text block, with a tick | hand-drawn mark degraded — a soft smear as shipped, stroke still followable |
| 0.0016 | `Xin Qu et al_2018` p28 | 87 px | two specks | nothing |
| 0.0016 / 0.0015 / 0.0014 / 0.0014 / 0.0014 | `Xin Qu et al_2018` p10 / p8 / p13 / p23 / p25 | 151 / 150 / 150 / 149 / 150 px | the **footnote separator rule** (a 2 px hairline, 75 px wide on four and 76 on p10, in three distinct positions across the five) | graphic rule blurred to grey |
| 0.0015 | `Riesman - 1954` p24 | 926 px | the left page-edge strip and two specks; its **margin band** additionally holds characters from the facing sheet | nothing of this page |
| **0.0008** | **`Jones et al_2010` p2** | **112 px** | **the word "value."** at the end of a short line | **LOSES ONE WORD** — an unresolved blob as shipped, with no letterforms; it reads `vaiuc.` only at the bar, so as published it is recoverable from context alone |
| 0.0000 | `Riesman - 1954` p12 | 9 px | — | nothing |
| 0.0000 | `Atkinson_1939` p1 | 15 px | scattered specks | nothing |
| 0.0000 | `Riesman - 1954` p10 / p14 | 2 / 1 px | — | nothing |
| 0.0000 | `Jones` p3 / p9, `Riesman - 1954` p6, `Xin Qu` p3 / p5, `_1973_Committee Against Racism_` p1 / p2 / p3 | **0 px** | — | nothing, by count rather than by eye |

**The stencil-ink cross-check, which is the part that does not depend on anyone's eye:**

✅ **AND THE PUBLISHED PAGE WAS COMPOSITED FOR THE RISKIEST VERDICT, WHICH IS THE TEST SUB-STEP 1'S
AUDIT ASKED FOR — AND IT CORRECTED THIS SECTION'S OWN FIRST DRAFT.** The page a reader sees is the
stencil over the background, not the background alone, so a mark read as lost off the background could
be rescued by stencil ink on top of it. Composited at 1:1 on `Jones` p7
(`magick bg-shipped.jpg -resize WxH! stencil.png -compose Multiply -composite`, **88 unique grey
levels**, which is the check that it is a real composite and not last session's flat-field failure):
the prose above and beside the equation is **sharp**, the equation number `(2)` is sharp, and — ⛔
against what the first draft of the table above claimed — the `ε_it ~ iid(0, σ²); i = 1,2,…,N;
t = 2,3,…,T` line **also survives sharp**, which its 2.87% stencil ink already said. What is illegible
in the composite is the **two displayed lines of the equation** (0.00% stencil ink) and the
`(p-value 0.12).` parenthetical lower down, and the running-head rule is simply absent. So the verdict
holds and is narrower than first written: two equation lines and a p-value, not the whole display.
Caught by running the test rather than by reasoning about it.

| rect | stencil ink | adjacent control, in the stencil |
|---|---|---|
| `Jones` p7, the two equation lines — `470x56+100+742` | **0.00%** | 15.59% (the prose line immediately above, `470x24+100+714`) |
| `Jones` p7, the `iid` line — `400x26+140+808` | 2.87% — **survives** | — |
| `Jones` p2, "value." — `46x7+640+831` | **0.00%** | 11.94% (the line above, `200x16+560+806`) |
| `Riesman - 1954` p20, the pen line — `90x420+330+2020` | **0.06%** | 17.69% (the type beside it, `300x420+440+2020`) |

**Read as counts: 2 of 24 lose content, 1 more degrades a hand-drawn mark, 5 blur a graphic rule, and
16 lose nothing a reader would see.** Sub-step 1's eight split 4 losing to 4 not; these 24 split 2 to
22 — and they interleave here too. Sorted by `inkOut` descending the 24 are ranked 1 `Broadhead` p4
(nothing), 2 `Broadhead` p2 (nothing), **3 `Jones` p7 (the equation)**, 4 `Riesman - 1954` p20 (the pen
line), 5-11 the `Xin Qu` pages and `Riesman` p24, **12 `Jones` p2 (the word)**, 13-24 the twelve pages
printing 0.0000 (nothing). So the two highest of the 24 lose nothing while ranks 3 and 12 lose type:
the same shape as the eight, one order of magnitude lower down.

⚠️ Two honest limits on that table. The first rect measured for `Jones` p2 read **12.26%** and was
simply in the wrong place — 30 px off, landing on stencilled type; the map's own bbox gave the rect
that reads 0.00%. And a 0.00% stencil in a rect has two possible causes, `region` excluding it or
Sauvola never calling it ink; the first is C28's mechanism and the second would be a different defect,
but **the harm is identical either way** — absent from the 1-bit layer, absent from the text layer by
fact 2, stored at 1/8.

⛔ **THE RESULT IS THAT THE LOSS REACHES A THIRD OF THE WAY TO ZERO.** Before today the lowest measured
loser was `Jones et al_2010` p5 at 0.0353. `Jones` p7 loses a paper's **estimating equation** at
**0.0137** — 3.3x below the shipped bar, 2.6x below the lowest of the eight near-misses — and `Jones`
p2 loses a word at **0.0008**. Displayed mathematics is a failure class this campaign had not seen:
Vision boxes prose and misses set equations, so the marks most reliably outside the stencil are exactly
the ones a reader cannot reconstruct. **17 of the 73 sit at or above 0.0137 and 27 sit at exactly
0.0000**, so "the pages near the bar" was never the population.

⛔ **AND THIS SUB-STEP COMPLETES THE PER-DOCUMENT PICTURE FOR ALL SIX, WHICH PRODUCES THE STRONGEST
RESULT IN THE CAMPAIGN: ONE SCAN DOES NOT ORDER ITS OWN PAGES.** The window-relevant set of a document
is the pages whose `verdict` at 0.08 is `all-text` — the ones the first term can move — and with these
24 every such page in all six documents has a verdict. ⚠️ **A first draft of this paragraph said "two
documents, and they are disjoint"; the adversarial review of this diff refuted it from the committed
sweep, and what it found is sharper than what was claimed.**

- ⛔ **`Riesman - 1954` has NO window.** Its **p18 at 0.0676 loses nothing** (C26 sub-step 4:
  show-through, fingers and the page edge) while its **p16 at 0.0565 loses a hand-drawn margin bracket
  outright** — the page that reproduces C26's founding failure mode. Any bar that protects p16 protects
  p18 as well, so **no value of the constant orders this scan's own pages**. That is a *within-document*
  inversion, and it is what finally answers the "the useful bar is per scan" idea sub-step 1 could only
  half-refute: it is false on the document with the most sampled pages in the six.
- `Broadhead - 1994`, all ten of its sampled pages the first term can move (p11 at 0.1034 and p13 at
  0.1210 are `picture` at either bar): 0.0767 / 0.0590 / 0.0465 / 0.0450 lose lines,
  0.0441 / 0.0410 / 0.0405 / 0.0370 / **0.0193 / 0.0140** do not — the two added today are the two
  lowest and they lose nothing, so the perfect ordering survives and the window is unchanged at
  **[0.04415, 0.04495]**.
- `Jones et al_2010`, all six (p4/p6/p10/p11/p13/p14 are already 1-bit and never reach this decision):
  0.0461 p12 loses table cells, 0.0353 p5 two lines of prose, **0.0137 p7 an equation**, **0.0008 p2 a
  word**, 0.0000 p3 / 0.0000 p9 nothing. Ordered — window **(0, 0.0008]** if the word counts,
  **(0.0008, 0.0137]** if it does not.
- Three more become computable from this sub-step's own rows: `Atkinson_1939` **(0, 0.0388]** (0.0580
  loses, 0.0388 loses, 0.0000 nothing), `_1973_Committee Against Racism_` **(0, 0.0779]** (0.0779 loses
  seven lines, 0.0000 x3 nothing), `Xin Qu et al_2018` **(0.0016, 0.0752]** if its footnote rules do not
  count (0.0752 loses the correlation matrix, everything else at or below 0.0016).

⚠️ **And "disjoint" was true only of the pair first named.** `Jones` ∩ `Atkinson` ∩ `CAR` ∩ `Xin Qu` is
**(0.0016, 0.0137]** — non-empty; only `Broadhead`'s [0.04415, 0.04495] is disjoint from the rest, and
`Riesman - 1954` has no window to intersect. So the corpus argument is now three-legged: the eight
near-misses interleave *across* documents (sub-step 1), `Broadhead`'s window is disjoint from four
others' — protecting `Jones` p7 at 0.0137 also protects `Broadhead` p2 and p4, **+379,584 B** on two
pages of thumbs — and `Riesman - 1954` cannot be ordered by any bar at all.

**The byte price, on the 13 the seam can reach.** 499,203 B shipped → 2,065,879 B un-shrunk:
**+1,566,676 B, 4.14x, 120,514 B/page** — in line with the band's 4.54x and sub-step 1's 4.07x. Of
that, `Jones` p7 (the equation) is **+82,755 B**, so **94.7% of it does not buy back content**;
counting `Jones` p2's word and `Riesman` p20's pen line as worth paying for leaves **71.4%**.
⚠️ **Neither of those is comparable to sub-step 1's 57.4%, and the first draft of this paragraph
compared them anyway** — 57.4% is the share landing on pages that lose *nothing at all*, a different
question. On that criterion the figure here is **56.6%**: the five priced pages whose verdict is
"nothing" (`Broadhead` p2 +190,111, p4 +189,473, `Riesman` p12 +187,582, p24 +273,985, `Xin Qu` p28
+46,220) hold 887,371 B of the 1,566,676. So like for like it is **flat, 57.4% → 56.6%**, not "getting
worse"; what moves is that the money buying back *content* has fallen from four pages to one. Caught by
the adversarial review of this diff.

⚠️ **One structural note this sub-step is the first to quantify: `inkOutsideText` drops the outer
sixteenth on every side, and the shrink does not.** The band is excluded for a good reason (the platen
edge and the gutter are ink by any threshold and are not content, and counting them put every text page
of `Blacks in the City` over any usable bar), but it means the signal is blind to a sixteenth of the
sheet it decides the resolution of. Measured out-of-stencil ink in that band on these 24 runs to
**283,499 px** (`Broadhead` p2), **308,622** (p4) and **176,290** (`Riesman - 1954` p24, which includes
characters from the facing sheet). ⚠️ **A first draft said "nothing belonging to the page itself was found there"; the review of this diff
found a counter-example and the claim is withdrawn.** Of the six bands over 50,000 px
(`Broadhead` p2/p4, `Riesman - 1954` p24 **176,290**, p10 **121,650**, p6 **73,525**, p20 **52,483**),
five hold only platen edge, gutter, thumbs and — on p24 — the facing sheet's type; but
**`Riesman - 1954` p6's holds a discrete typographic mark** (a heavy glyph and a solid full stop at
about `70x20+1495+8`, above the `INTRODUCTION` head — plausibly a section number), which is none of
those things and has no verdict. So the band is not merely scanner furniture, and C26's one
founding-mode loss (`Riesman - 1954` p16's hand-drawn bracket) lived in an outer margin too.

**What remains of question 1: 41 of the 73, and the cheap place is exhausted.** 32 rendered (8 + 24),
41 unrendered in **15** documents — `Krippner__The political economy of financial exuberance` **10**,
`_1939_Former students to Board re Merriam_` **7**, `Ford_1941_Speech_` **6**,
`Stanford_1891_Jane Stanford opening day speech` **4**, then 14 pages in 11 documents at 1-2 each.
**15 of the 41 print `0.0000`**, which is a print and not a verdict — the seam is refused only where
`inkOut` is *exactly* 0, and `barDelta` at a bar below 1e-5 is the test. The map above reads them
either way, and is cheaper than a render pair. ⛔ **20 of those 41 were RENDERED on 2026-08-20 — the
section below — and 8 of the 20 lose content; ✅ the remaining 21 were rendered the same day, 1 of them
loses content, and that closes the 73.**

#### The twenty highest-`inkOut` of the remaining 41, RENDERED — 8 of 20 lose content, and the losers are a MIDDLE band with non-losers on both sides — MEASURED 2026-08-20

**The population, and why these twenty.** The section above says to pick by `inkOut` descending, so:
the **twelve** highest-`inkOut` pages of the 41 (`1976 - Regis McKenna Papers` p4 down to
`Merriam_1913` p2, 0.0287 to 0.0063), **plus the completion of the two documents where completing
them was nearly free** — `Ford_1941_Speech_` (all 6 of its sub-bar pages, 5 of them extra) and
`Stanford_1891_Jane Stanford opening day speech` (all 4, 3 extra) — because a whole document is the
only way to ask whether a bar orders that document's own pages, which is where sub-steps 1 and 2 got
their sharpest results. Twenty pages, **eleven documents, none of which had been looked at** (the
twelve highest span eleven too — `_1958_Executive Pay` contributes two; a first draft of this
sentence said ten and was contradicted by the word "eleven" three words later). Eleven
invocations, `rc=0` on every one, `INKDUMP` **7 files per page with none missing on all 20** (140
files), and `inkOut`, `layered` and `extent` reproduce `INKBAR-2026-08-19.tsv` **digit for digit on
all 20 rows** — checked by a script over the committed file, not by eye.

✅ **And all twenty are reachable through the override seam**, unlike sub-step 2's 11-of-24 floor:
every one printed a changed `barVerdict` at its document's bar, so every row below has both a byte
price and a render pair. The bars used were strictly below the lowest printed `inkOut` in each
invocation — 0.02 / 0.01 / 0.005 / 0.0005 / 0.00004 — which is the rule the tool's header states.

⛔ **THE HEADLINE IS THAT THE BAR'S ORDERING IS WRONG AT THE TOP OF THE RANGE: THE FOUR PAGES WITH
THE MOST INK OUTSIDE THE RECOGNISED WORDS LOSE NOTHING.** Sorted by `inkOut` descending, the twenty
read

```
n n n n L L n n L L L L L L n n n n n n
```

The eight losers span `inkOut` **[0.0051, 0.0165]**. Of the twelve non-losers, **four sit above that
whole span**, **six below it**, and **two inside**. `pageIsAllText()` is one-sided — a page is
protected exactly when its `inkOut` is *at or above* the bar — so those four are the first four any
bar protects as it comes down, and each of them loses only a staple-and-debris shadow, magazine column
rules, a book's gutter shadow, or a photograph's frame.

⚠️ **Two things this paragraph must NOT be read as saying, both caught by the adversarial review of
this diff.** (a) The loser set is **not contiguous**: `Ford_1941` p3 (0.0107) and `Stanford_1891` p3
(0.0104) sit inside the span and lose nothing. The *interval* is contiguous, the set is not, and a
first draft wrote "contiguous middle band" while listing the two exceptions in the same sentence.
(b) ⛔ **The "non-losers on both sides" shape is partly a SELECTION EFFECT and the first draft called
it "a stronger refusal of the bar than the interleaving was", which it is not.** All six non-losers
*below* the span are among the eight pages added by completing `Ford_1941` and `Stanford_1891` for
convenience rather than by the stated rule. The twelve pages the rule actually chose read
`n n n n L L n n L L L L` — four non-losers above every loser and **none** below — which is
sub-step 1's finding reproduced in a fresh population, not a new one. What survives as new is the
top of that string: in sub-step 1 the highest-`inkOut` page of the eight was a loser, and here the
four highest of the 41 are not.

⚠️ **AND THE COMPARISON WITH SUB-STEPS 1 AND 2 ON "THE BAR OVERPAYS" IS FLAT, NOT A WEAKENING —
a first draft of this section claimed a weakening off MISMATCHED BASES.** Over all twenty pages the
price is **861,180 -> 3,892,140 B, +3,030,960 B, 4.52x, 151,548 B/page**, split **45.8% on the eight
losers / 54.2% on the twelve that lose nothing** — and 54.2% is the figure comparable with sub-step 1's
57.4% and sub-step 2's 56.6%, both of which are over *all* their priced pages. Flat. The narrower
question — what a bar chosen to protect every loser costs — is **+2,225,619 B over the 14 pages at or
above 0.0051**, of which **62.4%** lands on losers and **37.6%** on six that lose nothing; that 37.6%
is over 14 pages, not 20, and comparing it against the two whole-sample figures is what produced the
false claim. On this narrower basis sub-step 2 reads **50.7%** (a bar at 0.0008 does not reach
`Riesman - 1954` p12's +187,582 B), so it is flat there too. Sub-step 2's own text already warns that
these two definitions are not comparable, and this diff made the mistake anyway.

⛔ **THE STEEPEST-RATIO PAGE BUYS NOTHING, FOR THE THIRD TIME IN THIS CAMPAIGN.**
`1976 - Regis McKenna Papers - Early Career, 1965-1981` p4 is **+350,632 B at 7.89x**, the steepest
ratio measured anywhere in it, and its out-of-stencil ink is **13 components over 400 px** (8 over
800): the largest are four tall thin strips of platen/binding shadow spanning x 43-137, a
page-corner fragment, and a staple-and-debris shadow at `115x36+420+118` whose rect holds **0.0000**
stencil ink and whose shipped background still shows the same shape. No type-shaped component at
either threshold — the five that only a 400 threshold sees were crop-checked at 1:1 by the review of
this diff and are strokes and specks. ⚠️ **It is NOT the dearest page of the twenty and a first draft
said it was**: `Doermann_1967` p21 costs **+361,465 B**, 10,833 B more, and *does* lose content, so
the parallel with `RIESMAN_1942` p10 — which was dearest in absolute bytes — holds only on the ratio.
On ratio it joins `RIESMAN_1942` p10 (+702,280 B, 6.47x, a scanner-edge strip) and
`Riesman - 1954` p18.

**The verdicts.** Every mark located by connected components of the `ink AND NOT dilate(stencil)` map
— so the counts are measured, not impressions — and every verdict on a *loss* settled by a **four-row
1:1 crop: source / stencil / background at the bar / background as shipped.** The stencil row is the
decisive one: where it is blank, the background is the only carrier, and the fourth row is what a
reader gets. ⚠️ **Three of the twenty carry no 1:1 crop and rest on the map alone** —
`_1967_Ampex` p1 (column rules), `UN-OCred` p7 (a gutter/spine shadow) and `Stanford_1891` p3
(specks and one edge blob), none of which holds a type-shaped component. `Ford_1941` p3 was in that
list until the review of this diff pointed out that its "loses nothing" is load-bearing for the
within-document argument below; it was then cropped at 1:1, and its largest out-of-stencil component
(`134x303+1508+183`, and a `129x183` twin) is the **curled sheet's own edge shadow** crossing the
page, with its typescript sitting crisply *in* the stencil. Verdict unchanged, now evidenced.

| `inkOut` | document | page | `layered` -> at bar | | what the shipped background destroys |
|---|---|---|---|---|---|
...
| 0.0104 | `Stanford_1891 … opening day speech` | p3 | 22,030 -> 79,773 | 3.62x | specks, one edge blob — **nothing** |
| 0.0087 | `_1958_Executive Pay` | p3 | 39,354 -> 120,957 | 3.07x | **TWO LINES of running prose** |
| 0.0081 | `_1958_Executive Pay` | p5 | 40,549 -> 112,148 | 2.77x | **ONE LINE of running prose** |
| 0.0069 | `_1939_Former students to Board re Merriam_` | p6 | 52,626 -> 280,384 | 5.33x | **a handwritten signature** |
| 0.0063 | `Merriam_1913_Clutter of Things` | p2 | 85,981 -> 280,049 | 3.26x | **words of body text (`husband` and 8 more marks)** |
| 0.0056 | `Ford_1941_Speech_` | p6 | 31,137 -> 211,656 | 6.80x | **the page folio `-6-`**, and part of an ink stamp |
| 0.0051 | `Ford_1941_Speech_` | p1 | 47,782 -> 261,383 | 5.47x | **a pencilled marginal annotation**, and part of an ink stamp |
| 0.0023 | `Ford_1941_Speech_` | p2 | 47,489 -> 259,478 | 5.46x | stamp arc, specks — **nothing** |
| 0.0022 | `Ford_1941_Speech_` | p5 | 44,435 -> 249,553 | 5.62x | stamp arc, specks — **nothing** |
| 0.0012 | `Stanford_1891 …` | p1 | 25,636 -> 103,993 | 4.06x | specks — **nothing** |
| 0.0007 | `Ford_1941_Speech_` | p4 | 42,323 -> 243,575 | 5.75x | stamp arc, specks — **nothing** |
| 0.0005 | `Stanford_1891 …` | p2 | 19,257 -> 70,962 | 3.68x | specks — **nothing** |
| 0.0001 | `Stanford_1891 …` | p4 | 20,668 -> 77,588 | 3.75x | specks, one edge bar — **nothing** |

**Three of the eight are failure classes this campaign had not seen.** `Ford_1941` p6's **page folio**
— the characters `-6-`, in a rect holding **0.0000** stencil ink, legible at the bar and an unreadable
blur as shipped — and a folio is what a citation refers to. (⚠️ A first draft called it "the first
typeset mark lost that is not part of a sentence or a table"; sub-step 1's `Scott_TK` p3 lost the
*"Digitized by Google / Original from UNIVERSITY OF MICHIGAN"* footer, which is also neither.)
`Ford_1941` p1's **pencilled annotation** and `Doermann_1967` p21's **hand-inked margin number** are
archival marks a reader of a 1941 speech transcript or a 1967 thesis would want, and both rects also
hold **0.0000** stencil ink. And **magazine body text** is new: on `_1958_Executive Pay` p3 and p5
Vision boxes the lines above and below and misses one or two in the middle of a justified column.
(⚠️ Also drafted as a "first" — `Merriam_1913` is a `magazineArticle` too and loses a word of body
text in this same batch, so the two Executive Pay pages are only the first *found*.)

**The stencil cross-check, on tight rects.** The rect drawn around the lost mark alone, against the
adjacent line that survives:

| page | mark | stencil ink in the lost rect | adjacent survivor |
|---|---|---|---|
| `Herbert Marks` p11 | `City of Seattle` | **2.97%** | 7.80% |
| `Herbert Marks` p11 | `etc.` | **0.00%** | — |
| `_1958_Executive Pay` p3 | two prose lines | **2.56%** | 32.9% |
| `_1958_Executive Pay` p5 | one prose line | **6.42%** | 23.9% |
| `_1939_Former students` p6 | the signature | **0.43%** | 20.7% |
| `Merriam_1913` p2 | `husband` | **15.7%** | 33.8% |
| `Ford_1941` p1 | the annotation | **0.00%** | — |
| `Ford_1941` p6 | the folio | **0.00%** | — |
| `Doermann_1967` p21 | the margin number | **0.00%** | — |

⚠️ **`Merriam_1913` p2 is the weak row and it is labelled as such**: 15.7% against 33.8% is a ratio of
0.47, against **0.00-0.38** for the others — `Herbert Marks` p11's `City of Seattle` is 0.029728 /
0.0780097 = **0.38**, so Merriam is a far less clear outlier than a first draft's "0.00-0.27" made it
look. The cause is that a 23 px band on a 2807 px page catches the neighbouring lines' ascenders. Its
verdict rests on the crop, where the stencil holds broken dashes where the word
is and the shipped background holds an unreadable smear — not on this row. ⚠️ And the first version of
this table was worse: it measured the *display* rects, which deliberately include the surviving lines
above and below so a reader can see the contrast in one image, and those read 6.1% to 13.0% — not
loss-only figures at all. They were replaced with tight rects before publication.

**Within a document, the bar still cannot avoid a false positive.** `Ford_1941_Speech_`'s six pages
read `n L L n n n` by `inkOut` — its p3 at 0.0107 loses nothing while its p6 and p1 at 0.0056 and
0.0051 lose a folio and an annotation — so any bar protecting either loser also protects p3, at
+200,561 B. That is the same shape as sub-step 2's `Riesman - 1954`: in both, a non-loser sits at
strictly higher `inkOut` than a loser inside one scan, so no bar orders that document's own pages
without waste. It is now **two of the nine documents that have two or more still-shrunk pages, all
nine of which have now been read in full** (⚠️ a first draft said "two of the eight"; the six
sub-step 2 named plus `Ford_1941`, `Stanford_1891` and `_1958_Executive Pay` is nine).
`Stanford_1891`'s four all lose nothing and `_1958_Executive Pay`'s two both lose, so
both order trivially and neither is evidence either way.

⚠️ **TWO INSTRUMENT CORRECTIONS, AND THE FIRST ONE INVALIDATED THE LOCATOR'S FIRST RUN.**

1. **The map's published RECIPE has to crop to the interior — both images — because `inkOutsideText`
   does.** The guard walks x ∈ [w/16, w−w/16) and y ∈ [h/16, h−h/16) and divides outside-ink by
   *interior* ink, so it never sees a photographed surround or a platen edge. Counted over the whole
   page the locator's fraction reads **0.4479** on `Ford_1941` p1 against an `inkOut` of 0.0051 and
   **0.8199** on `Regis McKenna` p4 against 0.0287 — and the largest *ratio* is neither of those but
   **256x**, `Stanford_1891` p4's 0.0256 against 0.0001, with `Ford_1941` p4 at 143x (⚠️ a first
   draft led with "88x", which is Ford p1's ratio and only the largest absolute *fraction*). Cropped,
   the twenty read **0.56x to 16.0x**. ⚠️ **Two corrections to how this is stated.** (a) The crop must
   be applied to the ink image as well as the map — the fraction is outside/ink, and cropping only the
   numerator gives 0.0048 rather than 0.0087 on `Ford_1941` p1, an error of 1.8x in the direction that
   hides the problem; the first version of the recipe added to `score-text-route`'s header cropped
   only the map. (b) **Sub-step 2's own numbers were ALREADY interior** ("9.1% of its interior ink",
   "4.48% of its interior ink"), so its 0.97x-19.4x band is *not* void and is still the reference used
   above — it was only the shell recipe it published that omitted the crop, and that is what this
   sub-step followed. ⚠️ And what the outer band holds is **not established here**: `Ford_1941` p1's
   band carries 224,288 px of out-of-stencil ink and none of the twenty outer bands was inspected mark
   by mark, while sub-step 2 explicitly *withdrew* the claim that such a band holds nothing belonging
   to the page (`Riesman - 1954` p6's held a typographic mark). "Photographed surround and platen
   edge" is what these pages look like, not a measurement.
   ⚠️ **Six of the twenty read BELOW 1.0** — `_1967_Ampex` p1 0.56x, `Merriam_1913` p2 0.57x,
   `_1958_Executive Pay` p5 0.72x and p3 0.77x, `UN-OCred` p7 0.90x, `Gitlin_2000` p1 0.98x — and
   **the cause is not established.** A first draft attributed it to the `Disk:3` erosion eating marks
   within 3 px of stencilled ink; that is reasoned, and sub-step 2's own r=0-to-r=3 numbers move about
   2% on such pages, which cannot produce 0.56x. Two other differences are candidates and neither was
   isolated: ImageMagick's whole-page OTSU against `otsuThreshold(of:)`, and the fact that the dumped
   stencil is the Sauvola mask **∧ `region`** while the guard tests `region` alone — which should push
   the map *higher*, not lower. An r=0 interior run over these twenty would settle it and was not done.
   Either way the map is a locator and not a measure.
2. **The fraction interleaves as badly as the count did — the third measured refusal of a scalar, and
   the first on the fraction form.** Sorted by the locator's interior fraction the twenty read
   `L n n n L n n n L L L L L n n L n n n n`: `Merriam_1913` p2 loses words at 0.0036 while
   `Stanford_1891` p1 and p2 lose nothing at 0.0050 and 0.0041, and `Regis McKenna` p4 is second of
   twenty at 0.0339 and loses nothing. Sub-step 2 killed the pixel *count*; this kills its
   normalised form on the same page set that killed the bar.

⚠️ One sensitivity note, drafted as a cost note and upgraded by the review of this diff: on
`Regis McKenna` p4 (3024x3441) the connected-component call at `area-threshold=400` exceeded a
two-minute foreground ceiling and was re-run at 800 — **so the one page carrying both the steepest
ratio and the "no type on the map" headline was measured at half the sensitivity of the other
nineteen.** Re-run at 400 it has **13** components, not 8; the five that 800 hid were crop-checked at
1:1 (stencil ink 0.0000) and are a thin stroke and specks, so the verdict stands, but the count to
quote is 13 at 400. Corpus read-only throughout; all 140 dumped files, every map and every crop
written under `/tmp`.

✅ **What remained of question 1 — 21 of the 73, in 6 documents — WAS RENDERED 2026-08-20, and the
section after this one is the measurement.**
`Krippner__The political economy of financial exuberance` **10** (p10/13/16/19/22/25/28/31/34/37, all
at `inkOut` 0.0001 or 0.0000), `_1939_Former students to Board re Merriam_` **6** (p2/3/7/9/14/16),
`Guilford_Psychometric Methods` **2** (p1/p5), and one page each in
`Williams_1958_DEMOCRACY OR MERITOCRACY`, `Riesman_1973_THEMES OF WORK AND PLAY` and
`Herbert Marks papers` (p12). ⚠️ **The "same scan, same recogniser" argument, offered here for 7 of
the 21 on the strength of `Herbert Marks papers` p11 and `_1939_Former students` p6, returned
1 of its 7** — `_1939` p2's pencilled annotation, on the same scan as p6's signature; `Herbert Marks`
p12 and the other five `_1939` pages lose nothing, and this population's *other* loss, the typeset one,
is on `Williams_1958` p1 in a document nobody had opened. So "take the pages in documents already known
to lose content first" is worth something and is not a filter. ⚠️ **This paragraph read "came back
NEGATIVE on all seven" until the second review round of this diff**, which is the same slip as the "of
the 44" tally two sections down: p2 was found late, by the adaptive map, and two paragraphs written
before that were not brought forward. **15 of the 21 print `0.0000`** — `Krippner` 9 of its 10 (p10 prints 0.0001),
`_1939_Former students` 4, `Guilford` p5 and `Riesman_1973` p5 — and measured, **14 of the 21 are
refused by the seam**, `_1939` p14 being the one that prints `0.0000` and flips anyway.

#### The last 21 of the 73, RENDERED — question 1 is CLOSED: 16 of the 73 lose content, and the page holding the MOST ink outside the recognised words loses nothing — MEASURED 2026-08-20

**This is the fourth and final sub-step, and it finishes the population.** 8 + 24 + 20 + 21 = 73, so
every page still shrunk 8x/16x at the shipped bar has now been rendered at both factors and read.
...
on a shadowed photographed sheet falls on the paper side of it. See the third paragraph from the end.

**The population, and why these 21.** The 21 of the 41 that sub-step 3 left. ⚠️ **Not "the 21
lowest"** — the review of this diff reconstructed both sets from the committed TSV and they interleave
over ranks **15-26**: sub-step 3 completed two documents for convenience, so `Ford_1941` p2/p4/p5 and
`Stanford_1891` p1/p2/p4 were rendered there at `inkOut` **0.0001-0.0023** — six pages, each below at
most four of the 21. ⛔ **And `Ford_1941` p2 is the sharp one of the six**: it ties `Williams_1958`
p1's printed `inkOut` of 0.0023 exactly and loses nothing, which says what this sub-step's own
interleave says, at zero separation. (⚠️ A first correction here said "`Ford_1941` p4/p5 …
0.0001-0.0022, below six of the 21", which dropped p2 — the one page that carries the argument — and
counted a tie as "above"; four is the maximum. The second review round re-derived it from the TSV.)
The six documents are:
`Krippner__…financial exuberance` 10, `_1939_Former students to Board re Merriam_` 6 (p2/3/7/9/14/16),
`Guilford_Psychometric Methods` 2, and one page each in `Williams_1958_DEMOCRACY OR MERITOCRACY`,
`Riesman_1973_THEMES OF WORK AND PLAY` and `Herbert Marks papers` (p12). Six documents, `inkOut`
spanning **0.0000 to 0.0041** — the bottom of the 73's range — with **15 of the 21 printing 0.0000**.
Six invocations, `rc=0` on every one, `INKDUMP` **7 files per page with none missing on all 21** (147
files), and `inkOut`, `layered` and `extent` reproduce `INKBAR-2026-08-19.tsv` **digit for digit on all
21 rows**, checked by a script over the committed file rather than by eye.

⛔ **THE HEADLINE IS THAT THE BAR IS NOW REFUTED FROM BOTH ENDS OF ITS OWN RANGE.** Sub-step 3 found
that the four pages of the 41 with the *most* ink outside the recognised words lose nothing. This
sub-step supplies the other end. The typeset loss here, `Williams_1958` p1, sits at `inkOut` **0.0023**,
and **44 of the 73 have a strictly lower `inkOutsideText` than it does — 42 of which lose no content
and two of which do**: `Jones et al_2010` p2 at **0.0008**, from sub-step 2, 2.9x below, and this
sub-step's own `_1939_Former students` p2, which prints `0.0000`.
⚠️ **A first draft of this paragraph said all 44 lose nothing, and the review of the diff refuted it
from this entry's own sub-step 2 table** — where p2 is listed as *"LOSES ONE WORD"*, and the entry's
preamble already says "one page at 0.0008 is already losing type". ⚠️ **A second draft then said
"1 loses content … so 37 lose nothing", which forgot that this sub-step's own second loser is itself
one of the 44**; the review of *this* diff caught that. Of the 44, **2 lose content, 1
degrades a hand-drawn mark and 5 blur a footnote rule, so 36 lose nothing.** The claim that holds is the
stronger one: **the loss does not stop at the bottom of this sub-step's range** — it reaches 0.0008 on
one scan and, on `_1939_Former students` p2, a page whose `inkOut` **prints `0.0000` and which no
usable bar therefore reaches**. Within this sub-step, sorted by `inkOut`
descending, the twenty-one read

```
n n L n n n n n n n n n n n L n n n n n n
```

— `Williams` p1 is third; `_1939` p2 is drawn fifteenth but its true rank is **undetermined**, because
15 of the 21 tie at a printed 0.0000 and this table's order within that tie is the driver's, not the
page's. `Herbert Marks` p12 (0.0041) and
`Guilford` p1 (0.0031) sit above the first loser and lose nothing. ⚠️ **Stated in one frame, because a
first draft mixed two and counted 22 pages out of 21**: at the *shipped* bar 19 of these 21 are right
and the two losers are wrong; a corrective bar at or below 0.0023 rescues `Williams` p1 but wrongly
un-shrinks `Herbert Marks` p12 and `Guilford` p1, taking it to 18 right — and it still cannot express
`_1939` p2 at all, which is the point.

**7 of the 21 are reachable through the override seam and 14 are not**, which settles a corpus figure
this entry had been carrying as an inference. ⚠️ **It is 25 of the 73 that cannot be priced, not 27.**
27 rows print `inkOut` `0.0000`, and two of them flip at `INKBAR=0.00001` — `Riesman - 1954` p12
(sub-step 2) and `_1939_Former students` p14 (this one, +205,985 B) — so their true values are in
[0.00001, 0.00005) and the printed zero was never the test. 11 + 14 = 25, by addition over the two
sub-steps that met them. `INKBAR=0.00001` was used for all six invocations here, because the smallest
legal bar flips the most pages: the verdict is `inkOut < bar`, so a page is *un*-shrunk exactly when
its `inkOut` is at or above the bar.

**The byte price, and it is the worst overpay of the four sub-steps.** The 7 priced pages cost
**377,587 -> 1,684,507 B, +1,306,920 B, 4.46x, 186,703 B/page** — the highest per-page figure in the
campaign, because five of the seven are large typescript or newsprint sheets. Of that, **34.5% lands
on the one page that loses content and 65.5% on the six that do not**, against 57.4% / 56.6% / 54.2%
for sub-steps 1-3 on the same whole-sub-step basis. Over the whole 73 the 48 pages the seam can price
cost **+7,227,953 B** to un-shrink, 150,582 B/page (1,323,397 + 1,566,676 + 3,030,960 + 1,306,920, by
addition); the other 25 are unpriced and unpriceable.

**The verdicts.** Every mark located by the `ink AND NOT dilate(stencil)` map off the dump's own
`-source.png` and `-stencil.png`, interior-cropped on both images, with the two image sizes asserted
equal per page (they are, on all 21). `outPx` is interior out-of-stencil ink at erosion r=3; `frac` is
that over interior ink.

| `inkOut` | page | price | `outPx` | `frac` | `frac`/`inkOut` | verdict |
|---|---|---|---|---|---|---|
| 0.0041 | `Herbert Marks papers` p12 | 23,452 -> 92,052 (3.93x) | 5,178 | 0.04775 | 11.65x | nothing — a pale typescript's rim, legible as shipped |
| 0.0031 | `Guilford_Psychometric Methods` p1 | 29,174 -> 85,874 (2.94x) | **0** | 0.00000 | 0.00x | nothing, by count |
| **0.0023** | **`Williams_1958_DEMOCRACY OR MERITOCRACY` p1** | **137,185 -> 587,419 (4.28x)** | **1,493** | 0.00129 | 0.56x | **LOSES TYPE: the words `their education,` and `but,`** |
| 0.0013 | `_1939_Former students` p9 | 50,498 -> 253,455 (5.02x) | 2,112 | 0.00709 | 5.45x | nothing — typescript rim; `addressing your Excellency, I am not` reads at 1:1 |
| 0.0001 | `_1939_Former students` p7 | 49,893 -> 272,921 (5.47x) | 1,953 | 0.00539 | **53.94x** | nothing — `who got it there into that mess. How` reads at 1:1 |
| 0.0001 | `Krippner` p10 | 35,477 -> 134,893 (3.80x) | 20 | 0.00008 | 0.79x | nothing, by count |
| 0.0000 | `_1939_Former students` p14 | 51,908 -> 257,893 (4.97x) | 57 | 0.00014 | n/a | nothing, by count |
| 0.0000 | `_1939_Former students` p3 | unpriceable | 864 | 0.00245 | n/a | nothing — 2 interior components, 777 px, the larger a 16x51 strip of the photographed sheet's own edge |
| 0.0000 | **`_1939_Former students` p2** | **unpriceable** | 667 | 0.00182 | n/a | **LOSES A HAND-MADE MARK: a pencilled annotation down the right of the sheet** — see below; the Otsu map holds only 4 components, 217 px, largest 23x7, and is blind to it |
| 0.0000 | `_1939_Former students` p16 | unpriceable | 60 | 0.00015 | n/a | nothing — **no** interior component at all |
| 0.0000 | `Guilford_Psychometric Methods` p5, `Riesman_1973` p5, `Krippner` p13 / p19 | unpriceable | **0** | 0.00000 | n/a | nothing, by count |
| 0.0000 | `Krippner` p16 / p22 / p25 / p28 / p31 / p34 / p37 | unpriceable | 1-34 | ≤0.00014 | n/a | nothing, by count |

**The one loss, and the evidence for it.** `Williams_1958_DEMOCRACY OR MERITOCRACY` p1 is a 1958
Manchester Guardian page. The map's 1,493 interior pixels are **not** scattered: 998 of them are 21
components inside one rect, `249x18+711+1490`, and 214 more are 4 components inside
`45x17+936+1508`. The map holds **1,544** white pixels page-wide, 1,493 of them interior and 1,521 in
components of ≥8 px — so those two rects on two adjacent lines hold **1,212 of the 1,521**, and the
remaining 309 are twelve specks of ≤75 px scattered elsewhere. Those two rects are the words **`their education,`** on
the line *"…their education, their occupation and…"* and the word **`but`** on the line below,
*"…their power, but according to their…"* — the comma belongs to *power*, and a first draft wrote
`but,` and mis-transcribed the first line's tail; corrected by the review of the diff against the crop. At 1:1, source / stencil / map / composite-at-bar /
composite-as-shipped: the source is sharp, the stencil is blank where those words are, the bar's
composite reads them, and **the shipped composite reads `▬▬▬ ▬▬▬▬▬▬ their occu` and `their power, ▬▬
accor`** — an unresolved smear with no letterforms. The composite is stencil-over-background (160
unique grey levels in the rect, which is the check that it is a real composite and not a flat field), so this is
what a reader gets, not the background alone.

**And the cross-check that does not depend on anyone's eye — a better one than this entry has been
using.** Rather than the stencil's ink fraction in the lost rect against an adjacent survivor's, take
the **stencil's ink over the *source's* ink in the same rect**, which normalises for how much ink is
there at all:

| rect | what | source ink | stencil ink | stencil/source |
|---|---|---|---|---|
| `249x18+711+1490` | `their education,` — lost | 45.07% | **9.62%** | **0.21** |
| `200x18+970+1490` | `their occupa` — survivor, same line | 47.36% | 51.64% | 1.09 |
| `45x17+936+1508` | `but` — lost | 39.22% | **6.80%** | **0.17** |
| `230x17+700+1508` | `their power,` — survivor, same line | 27.54% | 29.18% | 1.06 |

A survivor's stencil holds essentially all of its ink; the two lost rects' stencils hold a fifth of
theirs. ⚠️ **But it is a WITHIN-PAGE contrast, and the margin for any corpus bar on it is narrow and
rests on two pages.** `Herbert Marks` p12's densest map bands read stencil/source **0.40-0.47**
and its two unflagged control bands **0.52-0.57** (bands `533x20+526+907`, `722x19+655+974`,
`702x37+605+450`, `709x35+568+722`, `709x37+628+990`, `528x36+772+1083`; controls `600x30+300+600` and
`600x30+300+1300`), on a page that loses nothing. ⚠️ **This read "seven densest bands" and "thirteen
rects" while listing six and twelve; the review of this diff counted them.** The enumerable figure is
what stands — **six** bands, **twelve** rects — and whether a seventh band was measured and dropped in
transcription is not recoverable from what was recorded, so treat the twelve as the sample.
⛔ So over these twelve rects the ratio does **not**
interleave — sorted it reads `L L n n n n n n n n n n` and a bar at 0.3 orders all twelve — which is
what a first draft of this paragraph claimed and the review of the diff refuted. What can be said is
that such a bar would sit in a **0.21-to-0.40 gap established on two pages**, untested; and that the
...
holds the **highest** out-of-stencil fraction of all 21 — 0.04775, **37x** the fraction of the one
page that loses content and **11.65x its own `inkOut`** — and it loses nothing: it is a pale
typescript, so the page-wide Otsu the map uses catches a rim that the adaptive Sauvola stencil does
not, on every glyph on the sheet. Four bands were read at 1:1 (`AGE Corp. - Employment (HSM)`,
`…politan Edison Company`, `…kinsville Water Company - corresponde`, `…andum of Pending Legal
Matters assigned`) and all are legible as shipped, softer than at the bar. Its 209 components of
≥8 px have a **largest of 49 px**: there is no type-shaped mark on the page, only 2,575 px of rim in
209 pieces. ⚠️ **So `frac`/`inkOut` over the campaign's cropped maps now spans 0.00x to at least 35x** —
`Guilford` p1 reads an empty map against an `inkOut` of 0.0031, `_1939` p7 reads the top of the range —
widening sub-step 3's 0.56x-16.0x in both directions. ⚠️ **The table prints that top ratio as 53.94x
and it should not be read to four figures**: `inkOut` is printed to 4 dp, so a denominator shown as
0.0001 is anywhere in [0.00005, 0.00015) and the ratio is anywhere in (35.9x, 107.8x]. The review of
this diff sized that; the claim is ">35x". The map locates; it does not measure.

**The thirteen settled by count, and the count is robust to the erosion radius.** `Krippner` ×10,
`Guilford` ×2 and `Riesman_1973` p5 read 0-34 interior pixels at r=3. Because these are small renders
(1014x1614 and 920x1428) where a 3 px disk is proportionally large, the four densest were re-run at
**r=1**, the most permissive legal radius: `Krippner` p34 goes 34 -> 867 px, p10 20 -> 251,
`Guilford` p1 0 -> 133 — and the **largest interior component in any of them is 8-10 px** (a 5x3,
a 6x2, a 7x2). The four quiet `_1939` pages behave the same way at r=1 — p2 **955 px in 12** components, largest
`12x47` (309 px); p3 **1,372 in 33**, largest `19x53` (904 px); p14 195 in 13, largest 15x8; p16 120 in
13, largest 3x6 — the two largest being the photographed sheet's own edge. ⚠️ **A first draft quoted
646 and 468 in 11 and 32, which is each of those figures with its largest component silently dropped**,
in the paragraph whose point is that nothing type-shaped survives; caught by the review of the diff
re-running the command. Nothing type-shaped survives it, so the *Otsu* verdicts do not rest on the
radius — but see the next paragraph for what Otsu cannot see at any radius.

⛔ **AND THE MAP'S GLOBAL OTSU IS BLIND TO A PALE MARK ON A SHADOWED SHEET, WHICH IS HOW THIS
SUB-STEP'S SECOND LOSS WAS NEARLY MISSED.** `inkOutsideText` and this map both threshold the whole page
at one level, and `_1939_Former students` p2 is a photographed handwritten letter whose right-hand
margin runs 0.36-0.74 grey: the pencil script there is *paler than the page's own Otsu*, so the map
reports **44 px** in `140x205+1660+2125` and **1 px** in `180x120+1640+1580` where an **adaptive**
threshold (`-lat 25x25-8%`, then `AND NOT dilate(stencil)`) reports **2,069** and **1,371**, against a
stencil holding 46 and 43 px. At 1:1 the source shows continuous cursive strokes and the shipped
composite shows disconnected specks with no followable stroke — the same standard by which
`Atkinson_1939` p3's signature and `Ford_1941` p1's pencilled annotation were counted as losses.
⛔ **And this is the entry's sharpest page, because p2's `inkOut` prints `0.0000` and its `barDelta` is
`same`: production's own first term is blind to exactly the ink that is lost, so no *usable* value of
`textPageInkOutsideThreshold` reaches this page.** ⚠️ Stated exactly, since the seam bounds rather than
measures it: `barDelta same` at `INKBAR=0.00001` puts `inkOutsideText` in **[0, 1e-5)**. If it is
exactly 0 then the guard's strict `<` means only a bar of **0** refuses the shrink, and a bar of 0
refuses it on all 16,987 pages; if it is positive then the bar that reaches it is under 1e-5, some
**4,500x** below the shipped 0.045 and three orders of magnitude below the lowest this campaign has
priced. Neither reading leaves a bar anyone would ship, which is the claim — not that the numerator is
provably zero. The two failures are opposite and both belong
to a global threshold — it invents a rim on a pale typescript (`Herbert Marks` p12) and misses real
pale ink on a shadowed sheet (`_1939` p2). ⚠️ The adaptive map was run over all 21 pages by the review
of this diff and broke none of the other nineteen negatives: `Guilford` p1 reads 17 px, p5 0,
`Riesman_1973` p5 1, the ten `Krippner` pages 138-315 px with no component ≥150, and `Herbert Marks`
p12 10,328 px still with no component ≥150 — so those verdicts are now stronger than the Otsu map alone
made them.

⛔ **AND `Disk:0` IS NOT "NO EROSION" — measured, and it would have inverted a dilation sweep.**
`Guilford` p1 reads **0 px at `-morphology Erode Disk:0`, 133 px at `Disk:1`, 15 at `Disk:2`, 0 at
`Disk:3`**. A monotone erosion cannot rise from 0 to 133 and fall again, and
`-define morphology:showKernel=1` says why: **`Disk:0` prints a 9x9+4+4 kernel — radius 4** — against
3x3, 5x5 and 7x7 for `Disk:1/2/3`. So it is ImageMagick's default radius, *larger* than `Disk:3`; the
series is monotone once r=0 is read as r=4, and the r=0 column of such a sweep is mislabelled rather
than merely void. Start at r=1. (The kernel print is the review of this diff's measurement; a first
draft said only "some larger default".) This is the same shape as the `-compose Minus` error sub-step 2 recorded: a dilation sweep
whose counts move the wrong way is the instrument, not the page.

⚠️ **ONE MORE INSTRUMENT NOTE, FROM A SLIP MADE AND CAUGHT IN THIS SUB-STEP.** ImageMagick's
`-connected-components` reports coordinates in the frame of the image it is given, so a run over the
**interior-cropped** map needs `+mx+my` added to reach page coordinates and a run over the **whole**
map needs nothing added. Both were used here, the offset was added to both, and the result was a
confident reading of a region 200 px away from any flagged ink — where the map is genuinely blank and
the page genuinely loses nothing, which is a **false negative that looked like a careful negative**.
What caught it was a direct white-pixel count in the rect: `400x60+690+1478` holds 1,212 of the page's
1,544 map pixels and `40x40+1050+1700` holds **0**. So before believing a component rect, count the
...
of its range first (sub-step 3), and below it the pages that need nothing outnumber the ones that do
about 37 to 2, while a page that loses a pencilled annotation sits at `inkOut` `0.0000` where the bar
has nothing at all to say (this one).
...
1. ✅ **ANSWERED, AND OVER THE WHOLE POPULATION: all 73 were rendered 2026-08-20 in four sub-steps and
   ...
   `_1958_Executive Pay` p3 two prose lines and p5 one, `Merriam_1913` p2 a word of body text, and
   `Ford_1941` p6 a page folio (eleven items over ten pages) — **4 lose a hand-made mark**
   (`Atkinson_1939` p3's signature, `_1939_Former students` p6's signature, `Ford_1941` p1's pencilled
   annotation and `Doermann_1967` p21's hand-inked margin number), **6 more lose something** without
   losing content (`Riesman - 1954` p20's pen line degraded to a followable smear and five
   `Xin Qu et al_2018` footnote rules blurred), and **32 lose nothing a reader would see**.
   ...
   **18 type and 6 hand-made over 24** — ⚠️ **19 + 6 with one page in both as of 2026-08-21, and the 10 + 4 above is 11 + 4: see `#### A shape term, MEASURED`**. The four
   ...
   documents already known to lose content first — was tried in sub-step 4 and returned 1 of its 7**:
   `_1939_Former students` p2's pencilled annotation, on the same scan as p6's signature. The other
   loss was in a document nobody had opened, so the heuristic is worth something and is not a filter. Two instruments, and prefer the
   second:
   `INKBAR=<a bar at or below the page's own inkOut> INKDUMP=<dir> /tmp/score-text-route "<pdf>"
   <page…>` writes the grey render, the stencil and **both** tone-layer pairs from the same `mrcLayers`
   call the byte columns come from — but it **cannot reach a page whose `inkOut` is 0**, which is
   **measured over all four sub-steps at 25 of the 73**, not the 27 that print `0.0000` (two of those
   flip). The `ink AND NOT dilate(stencil)` map in the sections above needs only the dump's
   `-source.png` and `-stencil.png`, works on all of them, and is validated against a known
   positive/negative pair. ⚠️ Read C26 sub-step 4's method paragraph before believing any of it: an
   auto-levelled whole-page difference **misled twice**, and then **twice more on 2026-08-20**; the
   readings that stood were 1:1 crops, the stencil's own ink fraction in the rect, and un-normalised
   locators. ⛔ And read sub-step 4's last three paragraphs before driving the map: **its global Otsu
...
3. **Can a shape term admit missed text into the stencil without admitting pictures?** This is R56's
   ...
   all three of those are the **rim of recognised type** rather than noise. ⛔ **A dilated-`region`
   collar does not remove them as a REPLACEMENT for the rule — measured 2026-08-21,
   `#### The rim fix, MEASURED`** — ✅ **but as a second condition it takes the non-losers from 3 of 51
   to 1 of 51 with all twelve type-losers intact** (`lineN >= 1 AND rim1N >= 1`). So the type half of
   this question now has a candidate rule rather than a bare rim, and what it needs is the picture pages
   and an out-of-sample population, because the conjunction is post-hoc. ⛔ The half of this question
...
- `Xin Qu et al_2018` p20, in **the register's own rect** for the thirteen values
  ...
  accepts 1,256 and the accepted groups 1,062, reading **10 of the 13 values**. ⚠️ The **three** that
  drop out are the shortest rows, `0.09` and `0.04`, and the diagonal `1` at the head of the column —
  three components each, under the four-member minimum. (This sentence said "the two that drop out",
  which contradicts its own "10 of the 13" by one; the row-profile count in this section's review
  paragraph 2 is the authority, and this is the sibling it left uncorrected. **13 minus the number that
  drop must equal the number named** — the arithmetic is the check.) That
...
- **The term is blind to a hand-made mark, measured 2 of 2** (p16 s bracket and `Atkinson_1939` p3 s signature — the review of this diff corrected the 1 of 1 this line first carried). A signature, a pencilled annotation, a
  ...
  ⛔ **"BY CONSTRUCTION" IS REFUTED as of 2026-08-21 — see `#### The same shape term on PICTURES`.**
  The next bullet already downgraded it to "too strong" and named the case to try; that case was run
  and the term fires **17 and 11 groups on a pen-and-ink ornament** and 1 inside a cartoon. The
  measured statement is now: **0 on four hand-made marks** (p16's bracket, `Atkinson_1939` p3's
  signature, `1954 - Why` p6 and p7's cartoons) and **fires on three** (`1881 - Harry Wilcox` p2 and
  p6, `1954 - Why` p4). The practical warning above still stands — it is unreliable on hand-made
  marks in *both* directions, which is worse than blind.
...
- **`textish` is never called by the self-test**, so four of the six constants have no coverage; and
  ...
  ✅ **That case WAS tried, 2026-08-21, and it confirms this prediction** — a pen ornament fires 17 and
  11 groups (`#### The same shape term on PICTURES`), so "too strong" becomes outright refuted. This
  bullet is the reason to read a review's *predictions* and not only its corrections.
...
#### ⛔ The same shape term on PICTURES — it errs in BOTH directions over 10 pages, and two of its misses are pages whose content loss this register already measured — 2026-08-21

**This is question 3b, the validation gap the previous section named rather than waved at.** The answer
to "does it read 0 on a picture" is **no** — `lineN` is **≥ 1 on 6 of 10** picture pages, and not one of
the six accepted groups is type. But the sharper result is the other direction: **it reads 0 on two
pages whose loss C26 measured at 1:1**, which is the failure that costs content rather than bytes.
Committed as **`SHAPETERM-PICTURES-2026-08-21.tsv`**, 10 measured rows plus one SKIP, the section
above's 22 columns with `document` and `arm` prepended.

##### How the ten pages were chosen, written down before the run

Two arms, so that neither a sorted column nor this register's own labels chooses alone:

- **Arm A — systematic.** The `verdict=picture` rows of `INKBAR-2026-08-19.tsv` sorted by `inkOut`
  descending, top 8, capped at 3 pages per document. ⚠️ **The cap never bound** — the uncapped top 8 is
  the identical list — so "one photo book cannot be the sample" describes a safeguard that did not
  operate, and `Ibson_2006` still supplies 3 of the 10. Seven of the eight were measured; the first
  **SKIPs** (see the instrument notes).
- **Arm B — labelled by this project.** `1954 - Why` p4, p6 and p7, the three cartoons `C26` was opened
  on and the only line drawings this campaign has read at 1:1. Their INKBAR `verdict` is `all-text`:
  they were shrunk 8x until the 2026-08-19 bar move, and `#### The rendered proof on the founding pages`
  measured all three losing their drawings at that shrink.

⚠️ **Ten measured rows in FOUR documents** (`1954 - Why` 4, `Ibson_2006` 3, `Wilcox` 2, `Riesman - 1954`
1); the fifth document contributed only the SKIP. ⛔ **And "10 picture pages" flatters the sample.** Of
the ten: **3 are true photographic/halftone plates**, 2 are album openings carrying a pen ornament, 3
are the arm-B cartoons (text pages with a drawing), and 2 are effectively blank sheets. On the class the
previous section actually owed — a plate or halftone — the sample is **3 pages, of which 2 fire.**

##### The table

| page | what the out-of-stencil ink is | `inkOut` | `outPx` | `glyphN` | `txtShare` | `lineN` | `linePx` | `lineShare` | `topLine` |
|---|---|---|---|---|---|---|---|---|---|
| `1954 - Why` p4 † | cartoon — **loss measured** | 0.0540 | 5,983 | 2468 | 0.0742 | **1** | 59 | 0.0099 | `27x17+1121+685` |
| `1954 - Why` p6 † | cartoon — **loss measured** | 0.0493 | 4,851 | 2013 | 0.0736 | **0** | 0 | 0.0000 | — |
| `1954 - Why` p7 † | cartoon — **loss measured** | 0.0660 | 7,080 | 2255 | 0.1075 | **0** | 0 | 0.0000 | — |
| `1954 - Why` p10 | near-blank back cover + show-through | 0.9735 | 1,353,270 | 7 | 0.0000 | **0** | 0 | 0.0000 | — |
| `1881 - Harry Wilcox` p2 | pen-and-ink ornament | 0.9227 | 251,186 | 288 | 0.0401 | **17** | 1,694 | 0.0067 | `60x41+2771+1015` |
| `1881 - Harry Wilcox` p6 | pen-and-ink ornament | 0.9773 | 364,895 | 70 | 0.0279 | **11** | 1,594 | 0.0044 | `78x33+1400+734` |
| `Riesman - 1954` p2 | blank grey endpaper | 0.9446 | 2,389,079 | 85 | 0.0092 | **2** | 10,438 | 0.0044 | `444x154+366+3459` |
| `Ibson_2006` p61 | halftone plate | 0.9515 | 2,254,516 | 393 | 0.0006 | **1** | 477 | 0.0002 | `74x29+1396+771` |
| `Ibson_2006` p122 | photographic plate | 0.9949 | 359,906 | 68 | 0.0107 | **12** | 2,032 | 0.0056 | `202x48+84+873` |
| `Ibson_2006` p162 | photographic plate | 0.9978 | 602,580 | 54 | 0.0005 | **0** | 0 | 0.0000 | — |

† arm B. ⚠️ `linePx` is the total over *all* accepted groups on the page, not the `topLine` box: on
`Riesman - 1954` p2 (`lineN` 2) a recount of the `444x154` rect holds 10,069 px of the 10,438.

##### ⛔ The dangerous direction first: two false NEGATIVES on pages of measured loss

`1954 - Why` p4, p6 and p7 all lose their drawings at 8x — measured at 1:1 on 2026-08-20, *"a grey
smudge"*, *"an amorphous blur"*, *"every stroke is gone"* — and the term reads **1 / 0 / 0**. So on the
...
mechanism is the *grouping* — `lineMinimumMembers = 4` and `lineGapFactor = 3.0` — not the
...
The previous section's own adversarial review already downgraded that claim to "too strong" and named
the case to try next: *"four blobs of a broken pen stroke on a diagonal, ≤ 3 glyph-heights apart"*. This
run is that case and it confirms the prediction — **17 and 11 accepted groups on a pen-and-ink
...
⚠️ Every verdict here is a human reading a `magick -crop` of the run's own `-source.png` against its
`-lines.png` at 1:1 in the same frame — no coordinate conversion and no `-normalize`, which is what
makes it better than the four whole-page misreadings this entry already records, but it is an eye and
not a number. **None is type.** Four are picture and two are threshold noise:

- **Halftone screen.** `Ibson_2006` p61's single group is 477 px of the plate's own screen dots, in a
  region whose grey render is flat tone with a visible dot grid and nothing else.
- **Photograph grain.** `Ibson_2006` p122's twelve groups all sit *inside* the plate — a five-man track
  team, printed inverted — and the thresholded grain of faces and collars is what the rule accepts.
  ✅ **That page has a real caption line** (*"Figure 71. Studio portrait, 7½" × 9¼", five-man track team,
  ca. 1890."*) which Vision recognised, so it is in the stencil: the caption's own rows hold **0** map
  pixels and none of the twelve groups is on it. The whole-page `-lines.png` is the location control.
- **Pen ornament.** `Wilcox` p2 (17 groups) and p6 (11) are a photographed album opening carrying a
  calligraphic flourish; the accepted groups are the ornament's feather strokes. ⛔ **On p2 it is wrong
  in both directions**: a `620x220+2180+540` crop of `-lines.png` over the ribbon's hand-lettered
  copperplate *"Harry W. Wilcox"* holds **0** accepted pixels, while 1,694 land on the decoration
  page-wide. ⚠️ That crop covers one of the page's **two** lettered ribbons and clips the tail of
  "Wilcox"; the lower ribbon's window holds 305 px, which read against the source sit on its scalloped
  border rather than on the word. So the direction is measured on one word, not on the page.
- **Otsu speckle in a flat field.** `Riesman - 1954` p2's two groups total 10,438 px, the larger in a
  `444x154` box whose `-source.png` is featureless grey — a photographed endpaper. That rect reads mean
  **107.5**, standard deviation **2.88**, against a page Otsu of **107**: the field straddles the
  threshold by ±3 grey levels, so the page-wide Otsu turns it into speckle and ≥ 4 specks tile a band.
  This is the one firing page with nothing on it to lose.

##### Four more scalars refused, two of them what an implementer would reach for first

Sorted, over these ten pages:

- **`glyphN`** interleaves: 2,468 fires and 2,255 does not; 68 fires and 54 does not. So **"the page has
  too little recognised type to calibrate against" is NOT the mechanism**, however plausible it looks
  beside `glyphH` reading 72 px on `1954 - Why` p10 and 5 px on `Wilcox` p2.
- **`txtShare`** interleaves — the **highest** of the ten (`1954 - Why` p7, 0.1075) reads `lineN` 0,
  and the firing pages occupy ranks 3, 4, 5, 6, 7 and 8 of 10. ⚠️ A draft of this bullet said it
  "inverts at both ends"; it does not — the two **lowest** (`1954 - Why` p10 at 0.0000 and
  `Ibson_2006` p162 at 0.0005) both read 0, and `Ibson_2006` p61 at 0.0006 is the third-lowest. The
  refusal survives, the ordinal did not. Sixth refusal of a share.
- **`linePx` in absolute pixels** interleaves against the previous section's labelled set: `Riesman -
  1954` p2's 10,438 px of endpaper speckle exceeds the 7,533 px `Broadhead - 1994` p3's two genuinely
  lost prose lines occupy (both are per-page `linePx` totals from the same tool).
- **`topLine` width** interleaves too: 202 px here (`Ibson_2006` p122) against 95 px on
  `Xin Qu et al_2018` p20. ⚠️ That 95-px group is **not** one of p20's thirteen matrix values — it sits
  at x 115–210, y 276–287, disjoint from the register's own rect for them (`62x233+741+378`), and 95 px
  cannot fit a 62-px column. What it is was not established; the run's dumps are gone.

##### ⛔ The only column that separates is a SHARE — the family this entry has refused six times

`lineShare` is the one column that does not interleave across both samples: **0.0002–0.0099** on the six
firings here, **0.0000** on the six labelled non-losers, **0.2332–0.9937** on the six labelled
type-losers, so a bar anywhere in (0.0099, 0.2332) — a **23.56x** gap — separates 22 of the 23 pages the
term has now been run on. **It is recorded because it is what the columns do, NOT as a candidate**, and
there are five reasons to leave it alone:

1. **It is `txtShare` with a narrower numerator** — the identical denominator, the numerator narrowed
   from `textish` pixels to line-accepted ones. `txtShare` is this entry's fifth refused scalar and is
   refused again two subsections above. Anyone putting a bar on `lineShare` is implementing the seventh
   share in a register that has refused six, and the 13-page section's own structural argument is
   against the whole family: *"a share is a scalar … while `lineN` asks whether the marks tile a
   baseline."*
2. **It separates TYPE loss, not content loss.** The 23rd page is `Riesman - 1954` p16, a content-loser
   reading `lineShare` **0.0000** — dropped out of the "22" above, which is why the denominator is
   written as 23. Add `1954 - Why` p4/p6/p7 at 0.0099 / 0.0000 / 0.0000, all three measured losers, and
   a bar in that window puts **four of the 23 known content-losers on the "nothing at risk" side**.
3. **It was chosen after seeing the data**, unlike the five constants of the previous section — a fit,
   not a rule.
4. **The mechanism is the denominator.** Seven of the ten pages read `inkOut` > 0.9, so the map is
   nearly the whole page and any numerator is a small share by construction. The page that sets the
   0.0099 floor is `1954 - Why` p4, whose `outPx` is 5,983 — the smallest denominator among the pages
   that *fire* (p6's 4,851 is smaller still and reads 0). The gap is held open by exactly the case that
   stresses it.
5. **The case a wiring would meet is not in the sample at all.** A plate or figure sitting in the map of
   a page otherwise read as all text, *below* the bar, has zero pages here, and the corpus sweep offers
   no other: every layered page of `Ibson_2006`, `Wilcox`, `Findlay_1992` and `Countryman - 2006` reads
   `inkOut` > 0.4 (lowest `Wilcox` p5 at 0.4250, nine times the bar).

##### What bounds the harm — under ONE of the two seams this entry names

All ten pages are `barVerdict=picture` in `INKBAR-2026-08-19.tsv`, so **none is shrunk in production
today**. A term wired as a second condition inside `pageIsAllText()`'s true branch
(`inkOut < 0.045 AND lineN == 0`) is therefore never consulted on any of them, and the byte cost of
these six firings is zero — ⚠️ **derived from the sweep's verdict column, not measured, because nothing
is wired**, and on a margin of **1.10x**: the lowest `inkOut` of the ten is 0.0493 against the shipped
0.045.

⛔ **Under the OTHER seam the bound does not hold at all, and it is the seam question 3's own sentence
names** — *"admit missed text into the stencil … what must not happen is the whole page entering the
stencil, which is what `textRegionMask` exists to prevent"*. `textRegionMask` is called
**unconditionally** on every layered page (`Sources/Flattener.swift:2696`, `let region =
textRegionMask(...)` immediately after the Sauvola mask), picture-route pages included. So on these six
pages a shape term feeding that function would admit halftone dots, photograph grain and pen strokes
into the 1-bit JBIG2 stencil — R57's failure mode, and verbatim what that function's own comment exists
to prevent. Nothing here prices it. **The stencil seam is now the more expensive of the two, and that is
a result of this sub-step rather than a caveat on it.**

⚠️ **And the absence argument that would bound the sub-bar population is over the wrong set.** "All 73
sub-bar pages were rendered and no destroyed plate or halftone is among the 16 losers" does not say the
73 carry no plates: **a plate can survive 8x** — this entry's own p6 finding is that an 8x blur keeps
...
- **`inkOut` reproduces `INKBAR-2026-08-19.tsv` on 10 of 10 rows**, digit for digit; the tool's own
  `mapFrac == inkOut` identity held on all ten (exit 6 never fired, `rc=0` on all five invocations);
  `stenFrac >= mapFrac` on all ten; `--self-test` passes its 5 checks on the exact binary that produced
  the rows.
- ⚠️ **The first cross-check of that reproduction read 0 of 10, and the instrument was wrong rather than
  the tool**: the lookup keyed `"p" + page` against a column that already reads `p6`. §3, in a throwaway
  script this time.
- ⚠️ **The top row of `INKBAR` sorted by `inkOut` is a page with no recognised words at all.**
  `Levy and Temin - 2007` p6 prints `inkOut` **1.0000** there, which is `inkOutsideText` being 1 by
  degeneracy when `region` is empty; `score-shape-term` refuses it (`SKIP no words — mrcLayers refuses
  this page too`). So a reader sorting that sweep by `inkOut` gets, at the top, a page the layering
  decision never reaches, and arm A measured 7 pages rather than 8.
- ⚠️ **A shell pixel count of `-lines.png` does not equal `linePx`, and the divergence is up to 1.32x**
  — over the six firing pages `magick -threshold 50%` reads 1.000x (`Why` p4, 59/59), 1.05x (`Wilcox`
  p2, 1784/1694), 1.11x, 1.13x, 1.14x and **1.32x** (`Riesman - 1954` p2, 13781/10438), worst on the
  page the 10,438 figure comes from. Use the shell for "is anything accepted in this rect" — which is
  how the 0-pixel lettering result above was obtained — never as a total.
- ⚠️ **`1954 - Why` p10 was labelled "full-page photograph" in the first draft of this section and it is
  not one**: `tone` 0.992 in the sweep invited the guess, and the render is a near-blank back cover
  (mean 195.5, standard deviation 24.1) carrying a boxed "NFI" logo and heavy mirror-reversed
  show-through — `glyphN` 7 at `glyphH` 72 is the logo. Caught by the adversarial review of this diff,
  which read the crop. **A tone fraction is not a picture detector.**
...
- ~~The rim fix (subtract a dilated `region`) is named, not tried.~~ ⛔ **TRIED 2026-08-21: refused as
  a REPLACEMENT for the rule, ✅ open and best-measured as a SECOND CONDITION beside it** — see
  `#### The rim fix, MEASURED` below.
...
#### The rim fix, MEASURED over the same 73 — ⛔ as a REPLACEMENT for the rule it is refused, and at the one radius that keeps every real loss it MANUFACTURES a rim of its own; ✅ but as a SECOND CONDITION beside the r=0 rule it removes two of the three false positives at no cost to the twelve — 2026-08-21

⛔ **READ THE TWO READINGS SEPARATELY. The first draft of this section published only the first, called
the candidate "refused", and told the next session the cheap fix was spent** — the adversarial review of
that diff computed the conjunction from this section's own committed file and refuted the headline. What
is refused is *substituting* a collar radius for the shipped rule. Using it as a second condition is
measured, better than the shipped rule on this population, and open.

The section above named one candidate and called it the cheap thing to try first: *"subtract a
**dilated** `region` before grouping, so a rim is eaten and a missed line is not."* It is now in
`Tools/score-shape-term.swift` as a **sweep** rather than a knob — `rim1N`/`rim2N`/`rim3N`, the shape
rule re-run over the map with an `r`-pixel collar around every recognised word box removed — and it
was read over the same 73 pages, the same population, with the same five constants and one binary.
**`SHAPETERM-RIM-2026-08-21.tsv`** is the result: 73 rows, 22 documents, **36 fields** — the tool's own
31 columns (`page` through `verdict`), the driver's `document`, and the four labels carried from the
previous file.

⚠️ The collar is a *separate* mask and not a change to the map. `mapFrac`, `outPx` and every share
column stay `inkOutsideText`'s own set, which is what keeps the identity guard meaningful — and which
is why **every column the previous run printed reproduces it on 73 of 73 pages, digit for digit: all 27
shared columns**, `w`, `h`, `otsu`, `txtShare` and `lineShare` included. The rim columns are strictly
additive. ⚠️ A draft of this line said "the 16 columns the previous run printed" and enumerated them,
which undercounts twice: that run printed 22 columns, 21 of them measurements, and the enumeration left
out
`lineShare` — the "seventh share", published the day before, so exactly the column a reader would want
to know had not moved. The 27 were then checked one by one.

##### The cross-tab, by radius

| bucket | r=0 (shipped) | r=1 | r=2 | r=3 |
|---|---|---|---|---|
| loses **typeset** content | **12/12** | **12/12** | 11/12 | 9/12 |
| loses only a **hand-made** mark | 1/4 | 1/4 | 1/4 | 1/4 |
| degraded but legible | 0/6 | 0/6 | 0/6 | 0/6 |
| loses nothing | **3/51** | **2/51** | 1/51 | 1/51 |

**r=1 is the only radius that keeps every real typeset loss, and as a replacement it does not clear the
rim.** It removes two of the three false positives — `_1939_Former students` p9 (303 accepted line px ->
0) and `_1967_Yearly Increase` p1 (319 -> 0) — and **adds one of its own**, so 3 of 51 becomes 2 of 51.
`Xin Qu et al_2018` p28 is the **fourth** distinct non-loser to fire anywhere in this sweep and the only
one the collar itself produces.

##### ✅ The reading the first draft missed: the collar as a SECOND CONDITION

`lineN >= 1` **AND** `rim1N >= 1` reads **12/12** type-losers, 1/4 hand, 0/6 degraded and **1 of 51**
that lose nothing. It is strictly better than the shipped rule on this population and it is structurally
immune to the manufacturing above, because `Xin Qu` p28 has `lineN` **0** at r=0 and can never enter. The
conjunction is not a new radius: it is `rim1N >= 1` restricted to the pages the shipped rule already
fires on, and the two false positives it drops are exactly the two the collar clears.

| rule | type | hand | degraded | nothing | pages firing |
|---|---|---|---|---|---|
| `lineN >= 1` (shipped, r=0) | 12/12 | 1/4 | 0/6 | **3/51** | 16 of 73 |
| `rim1N >= 1` (replacement) | 12/12 | 1/4 | 0/6 | 2/51 | 15 of 73 |
| **`lineN >= 1 AND rim1N >= 1`** | **12/12** | 1/4 | 0/6 | **1/51** | **14 of 73** |
| `lineN >= 1 AND rim2N >= 1` | 11/12 | 1/4 | 0/6 | 1/51 | 13 of 73 |

The two pages the conjunction drops from the firing set are **both non-losers**, so the wiring's price
can only fall from the +2,362,625 B over 16 pages the section above measured and the 15.5% share landing
on pages that lose nothing can only improve. ⚠️ **That is a direction and not a number** — no bytes were
measured on this arm, and the two pages' own costs are not carried here.

⚠️ **Three reasons this is a lead and not a result.** (1) It is **post-hoc**: a conjunction picked after
...
`Xin Qu et al_2018` p28 reads `lineN` **0** at r=0, **2** at r=1, and **0** again at r=2 and r=3 — it
fires at exactly one radius and at no other. Its **larger** accepted group at r=1 is `rim1Top`
`26x6+132+835`, and cropped at 1:1 against `-source.png` it is the **top halves of `469.`**, a page
number in a reference line that Vision recognised (the page's whole out-of-stencil ink is `outPx`
**163**). ⚠️ There are **two** groups at r=1 and only this one was read. The second was located at
`y` ~1022 on the same dump and **not** read, and the tool prints only the page total (`rim1Px` **117**
across both), so no per-group area is on record for either. Connected components over the same rect on
the r=0 dumps say why the read one formed:

| | components in `x` 132-158, `y` 834-841 |
|---|---|
| r=0 `-map.png` | `8x8`, `8x8`, `7x8`, and a `2x3` speck |
| r=0 `-textish.png` | `8x8`, `8x8`, `7x8` — **three** accepted; the speck has area 4, so the area guard passes it and the *height* floor refuses it, `0.5 x glyphH` = 5 px against its 3 |
| r=1 accepted | `8x6`, **`4x6`, `3x6`**, `6x6` — **four** |

Three accepted components is one short of `lineMinimumMembers`. The 1-pixel collar **split the middle
one** (`8x8` -> `4x6` + `3x6`), took the count to four, and the group formed. ⚠️ Note the heights go
8 -> 6 under a **one**-pixel collar, which means these stubs sit *between* two word boxes and lose a row
to each — the line above's box and their own. ⚠️ And these bounding boxes are **not in any committed
file**: the tool prints only `rim<r>N/Px/Top` per radius, so they were read by running ImageMagick
connected components over the uncommitted `SHAPEDUMP` PNGs. `rim1N` = 2 and `rim1Px` = 117 are the
committed numbers. This is the
non-monotonicity the tool's comment predicted before the run, measured: **cutting the collar can turn
a rim that was too sparse to group into one that groups.** The candidate does not merely fail to
remove the artefact; on one page of 73 it is the artefact's cause.

⚠️ It happens on a real loser too, in the harmless direction: `Herbert Marks papers` p11 reads `lineN`
1 -> 1 -> **2** -> 1 while its `linePx` falls monotonically 423 -> 377 -> 311 -> 114. So a count that
goes up while the pixels go down is the *normal* behaviour of a collar and not a fault of p28 — which
is why each radius gets its own components pass in the tool rather than a filter over the r=0 ones.

##### ⛔ The false positive that survives every radius is still the rim at r=3

`Herbert Marks papers` p12 goes `lineN` 2 -> 1 -> 1 -> 1 and `linePx` 386 -> 145 -> 94 -> **64**. Its
surviving group at r=3 is `65x4+606+914`, and read at 1:1 (`-crop 150x40+590+900`) it is **4-pixel
flecks off the tops of `Corp.`** on a typed ledger line that is in the text layer. So the collar erodes
this rim by 6x without clearing it.

##### ⛔ And from r=2 it destroys real losses

- `Williams_1958_DEMOCRACY OR MERITOCRACY` p1 — the words `their education,` and `but`, at `inkOut`
  **0.0023**. `linePx` 1780 -> 1487 -> **0**. Gone at r=2. ⚠️ A draft of this line called it "the
  lowest-`inkOut` typeset loss the campaign has", which is false and false from this section's own
  file: `Jones et al_2010` p2 is a typeset loss at **0.0008**, 2.9x lower, and is one of the twelve
  tabulated above. It survives every radius (147 -> 133 -> 119 -> 100).
- `Scott_TK` p3 — table figures, column heads and Roman row labels. 6 groups -> 2 -> 1 -> **0**, and
  `linePx` 1796 -> 278 -> 115 -> 0. Gone at r=3.
- `Merriam_1913` p2 — 5 groups -> 4 -> 1 -> **0**, `linePx` 4256 -> 2414 -> 489 -> 0. Gone at r=3.

##### The mechanism, and why a ratio-scaled collar is refused too

**The collar is absolute pixels where every other number in this rule is a ratio against the page's own
type. Scaling it to `glyphH` does not fix that, and the sharpest pair says so directly:** `Scott_TK` p3
loses its last group at r=3, which is **0.375x its own `glyphH`** (8), while the surviving false positive
`Herbert Marks` p12 still fires at r=3, **0.6x its own** (5). A ratio big enough to clear p12 has already
destroyed a real loss elsewhere. ⚠️ Not "on a page of smaller type", which a draft of this line said and
which is backwards: `Scott_TK` p3's `glyphH` is **8** against p12's **5**, and that is the point — at a
fixed ratio the larger `glyphH` buys the larger absolute collar, so the page with *more* type scale is
the one a ratio eats first. `Williams_1958` p1 is the same argument at the other
end — gone at r=2, **0.095x** its `glyphH` of 21.

⚠️ **A draft of this paragraph said the mechanism was that "the collar runs backwards" across the
population — smallest type where the false positive is, largest where the true positives are — and the
adversarial review refuted it from the same file.** Two facts kill that framing. **Three** type-losers
are destroyed, not two, and the third is `Scott_TK` p3 at `glyphH` **8**, the second-smallest of the
twelve — so the destroyed pages are not "the largest". And `Herbert Marks` **p11**, a real typeset loser
in the same document at the same `glyphH` **5**, fires at *every* radius — so "the population's smallest
type" is shared by a false positive and a true positive and separates nothing. The conclusion survives
and is stronger without the framing; the framing was a mechanism story built on a chosen pair.

⚠️ **All of it is derived from the measured radii 1-3, not run**: radii above 3 were not measured, and
the derivation assumes p12 needs r >= 4 (measured: it fires at 3) and that a destroyed group does not
come back at a larger radius (unmeasured, and the sweep's own non-monotonicity is a reason not to assume
it — coming back would be stranger still, but "stranger" is not "measured").

##### What this changes for the local variant, which is the one the entry prefers

The wiring priced above is the boolean `lineN == 0`, and for that only firing matters. The variant this
entry says it prefers — exempt the *text-shaped regions* from the background shrink rather than refuse
the whole page — reads the **rects**, and a collar is much worse there than the boolean shows. At r=1
`Scott_TK` p3 keeps **278 of 1,796** accepted line pixels (15.5%) and its largest group **moves** from
`241x5+364+150` to `56x6+283+487` — a different part of the page. `Merriam_1913` p2 keeps 2,414 of
4,256 and its largest group moves from `271x11+113+417` to `132x11+1482+1099`. So even at the one
radius where every real loss still *fires*, the regions a local exemption would protect are a fraction
of the ink that needs protecting, in a different place.

##### Controls

- `rc=0` on all **22** invocations; **73 of 73** rows measured; `verdict=ok` on every row, so the
  `mapFrac == inkOut` identity held and exit 6 never fired.
- All **27** shared columns are identical to `SHAPETERM-73-2026-08-21.tsv` on **73 of 73** pages, which
  is what says the rim columns did not perturb the measurement they are read beside. `inkOut ==
  sweepInkOut` on 73 of 73 as well, so `INKBAR-2026-08-19.tsv` is still reproduced through this file.
- The whole sweep was **re-run after the review's two code fixes** and all 22 per-document outputs came
  back byte-identical, so the committed file is the shipped binary's own output and neither the
  collar-width assertions nor the `SHAPEDUMP` paint fix touches a printed column.
- `--self-test` **6 of 6** on the exact binary. The new check is the rim rule on one synthetic scene and
  it is **two-sided**: a word box with five glyph-top stubs above it and, twenty pixels clear, four full
  stems standing for an unrecognised line. Untrimmed the rule must read **both** as lines; at r=3 it must
  read exactly the real one, identified by its `minY`. **Watched failing on four mutants**, each a
  one-line change to `rimSubtract`, each compiled and run. Two versions of the check exist and the table
  says which each mutant was run against, because the second version was written *because* of the fourth
  mutant:
  | mutant | against the check as first written | against the shipped check |
  |---|---|---|
  | returns the map untouched | **1** failure: `rim: r=3 read 2 lines not 1 — the rim survived or the real line went with it` | **4** failures (that one plus the three pixel-count rows) |
  | returns an empty mask | **3** failures (untrimmed read 0 lines, r=3 read 0, radius 0 changed the map) — which is what says the check is two-sided rather than a one-way "did anything go away" | not re-run |
  | dilates the *map* instead of the region | caught (r=3 read 0) | not re-run |
  | ⛔ `dilate(region, radius: r - 1)` — a collar one pixel too small | **passed 6 of 6** | **3** failures (`r=1 removed 0 px not 10`, and 10 and 20 where 20 and 30 are due) |
  ⛔ **That last row is the finding.** A collar built one pixel small would have relabelled this whole
  sweep by a column and made `rim1` a no-op, and the check as first written could not see it, because the
  scene's stub heights survive a 1-px collar either way — a check that looked green while not asserting
  the one number the sweep's columns are named after. The check now asserts the collar's *width*
  directly: `rimSubtract` must remove exactly 10, 20 and 30 px at r=1, 2, 3 on that scene. Found by the
  adversarial review of this diff, which built the mutant.
  ⚠️ The first stub attempt did not compile (it left a reference to the variable it had replaced), so it
  tested nothing and was rebuilt — a control that fails to build is not a control.
- ⛔ **One real defect in this diff, found by the same review**: `SHAPEDUMP`'s new `-rim<r>-lines.png`
  files were painted through the **untrimmed** map, so a surviving component's bounding box could show
  ink the radius had removed. Fixed in the same commit (`paint` takes the mask it found the components
  in), and the `Herbert Marks` p12 crop above was **re-read on the fixed build**: the r=3 file holds
  seven components of 3-4 px totalling **64 px, exactly `rim3Px`**, so on that page nothing had been
  over-painted and the reading is unchanged. The authoritative numbers were never affected either —
  `rim<r>N` and `rim<r>Px` come from the trimmed components, not from the PNG.
- The labels are unchanged: the campaign's published verdicts, carried from the previous run's file
  rather than re-read. The two pages cropped at 1:1 here are `Xin Qu` p28 and `Herbert Marks` p12, and
  neither reading changed a label — both are rim, which is what they were already recorded as (p28 was
  `nothing` and quiet at r=0).
...
- **The second group `Xin Qu` p28 manufactures at r=1.** Located at `y` ~1022, not read. Only the page
  total `rim1Px` 117 is on record for the two together.
- **Nothing is wired**, again: `Sources/` is untouched by this sub-step, so no mutant and no
  `fault-inject.sh` case is owed. Only `Tools/score-shape-term.swift` and its documents moved.
- **No bytes.** This arm is about which pages fire, not what a wiring costs; the byte price in the
  section above is unchanged by it, because r=0 is what that price was measured at.
- Whether the three false positives can be removed by something that is not a collar. Two facts bear
  on it and the first is **weaker than it looks**: three of the four rim firings (the three at r=0,
  plus the one r=1 manufactures) sit on pages whose whole out-of-stencil ink is tiny — `outPx`
  **437**, **381** and **163** — but the fourth, `_1967_Yearly Increase` p1, reads **5,863**, above
...
campaign (86 pages at 1:1, 24 losing content) is about the **background being stored at 1/8**, i.e. the
