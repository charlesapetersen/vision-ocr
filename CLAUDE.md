...
needs `gh ≥ 18.67` — so no value of it reds the check (⚠️ reasoned in the LOWERING direction, which is
what that argument is about and where it is still unrun; ✅ **the RAISING direction is measured green as
of the same day** — `const/shapeMinimumArea` 4 → 99 leaves `c28ShortGroups` at 0, those marks being
refused by area where they were refused by height). ✅ **So the term's six
...
✅ **AND THE SIXTH AND LAST CONSTANT GOT ITS FIXTURE THE SAME DAY — but the result worth quoting is the
run that came FIRST, and it is a `SURVIVED`** (`BUGS.md` C28 `#### The area floor's own fixture`, the
queue's `c28-area-fixture`). `shapeMinimumArea` is the only one of the six live in **two** places —
`textShaped`'s first guard (`Flattener.swift:2135`) and the calibration's `sized` filter (`:2267`), which
is upstream of every bar the other five are multiplied into — so the box asked whether it was already
pinned before building it anything. Run against the suite as it stood: **`SURVIVED`, 293 s,
`1371/1371 passed`, nothing objecting.** ⛔ **THE MEASUREMENT INSIDE THAT SURVIVAL IS THE FINDING: all
FOUR calibration readings held at 25.0 / 5.0 with the filter at 99, so the calibration does not move —
and it is informative only because of HOW the mirror is wired.** `c28Calibration` reads the constant
itself, so the two copies moved together exactly as its doc comment warns; what makes the comparison
mean anything is the **literal**, which is the third party. So that helper is blind where an assertion
compares the two copies and **not** blind against a literal. ⚠️ The *mechanism* (only the i-dots fall
under 99 px of ink) is reasoned — no component census was taken — and must not be quoted with the
medians, which are measured.
⛔ **A DRAFT SAID THE MUTANT IS MONOTONE IN THE REFUSING DIRECTION SO EVERY C28 CHECK ASSERTING ZERO
GROUPS IS UNFALSIFIABLE, AND THE REVIEW OF THIS DIFF REFUTED IT FROM THIS SUITE'S OWN TEXT** — the
standing test is *does the mutant change this check's INPUT*, and `textLines`' greedy banding makes
the count non-monotone in the accepted set, which this file already records. ⚠️ **The set is EIGHT
and the draft listed seven, omitting `c28BorderGroups`**; what survives is that every mark DRAWN on
them clears 99, that `c28TooShort` keeps its answer while the reason under it changes silently
(height floor at 4, area floor at 99), and that **`c28GroupsBoxed`'s green is NOT established**, its
page carrying rim ink the filter can drop. ⛔ **And the kill SURFACE was never three checks —
`const/lineMinimumMembers`'s row shows six, the three group checks plus three `c28Missed` WIRING
rows — so the survival is MORE informative than the draft claimed, not less.** ✅ The fixture that closes it is `c28Dashes(4, 16)`, four
marks every other term accepts (16 in [12.5, 75], `medianRun` 4 ≤ 10, gaps 57 ≤ 75, four members ≥ 4)
and only the area guard refuses: **`killed`, 294 s, baseline `1374 checks, green`, `1373/1374`, EXACTLY
ONE** check. ⛔ **Its kill set is a SINGLETON but the check is NOT exclusive, predicted before the run
...
suite's green interval for `shapeHeightLow` goes from a reasoned *at most (0.32, 1.2]* to a DERIVED
*at most (0.32, 0.64]*. ⚠️ The pair differs in TWO dimensions and cannot differ in one — area is
a product — so the attribution is the kill set's and never the geometry's; and the ink literals are
DERIVED from the too-short page's `T ≈ 114,800` model rather than read out of a deliberately-failing
run, which is a step down from that page's. ⚠️ Catalogue **107 → 108**, coverage **86 of 107 → 87 of
108**, census **still 21** a fourth time, log **105 → 107** (two rows, the `SURVIVED` kept because it is
what says the fixture was needed), suite **1,371 → 1,374**, `0 survivor(s)` unchanged; re-derive all of
them. Nothing in `Sources/` moved. ⚠️ Estimator: the **eleventh** reading and the **fifth** outside the
printed range — `10-10` again, measured **592 s**, 8 s under the 600 s floor — and the first of that
session's two runs is deliberately NOT counted, its start having been written by a previous tool call.
⛔ **It is A11.1's failure mode a FOURTH time and not a fifth: *fifth outside the range* and *fourth
degenerate printed range* are two counters, and a draft reused one ordinal for both.** ⛔ **And do not
write that the degenerate line accounts for every outside reading — four of the five are degenerate
and BELOW; the fifth is 2026-08-29's non-degenerate `9-10` measured at 800 s, 33.3% OVER.**
...
**four**, the fourth already unable to fail for an unrelated reason — ⛔ **and the DISARMED SET IS FIVE,
corrected 2026-08-30 by the next run: `Tests/main.swift:3255`, the mask pair, is not in that block and is
...
whole distribution is 11×1, 6×4, 5×5, 4×3, 3×23, 2×20, 1×34, so TEN rows are wider and THIRTY-FOUR
narrower** (a draft printed only the top four buckets, which carries the wide half of that claim and not
the narrow one). ⛔ **THE FINDING: THE SEAM'S TWO MUTANTS
HAVE DISJOINT KILL SETS** — `{:2258, :2531, :2548, :2551, :2561, :2618}` against
`{:2591, :2594, :2601, :2634}`, intersecting in nothing, read off both log rows and mapped to check lines.
⛔ **"Each other's complement" is NOT the mechanism and a draft said it was**: two of the const mutant's six
are in neither block, it kills 4 of the 5 `nil`-arm layering checks rather than the block, and one of this
mutant's four substitutes 0.15 rather than 0.08. ⛔ **AND THE ASYMMETRY IS THE ASSERTION *FORM*, NOT THE
DIRECTION — a draft wrote "only the constant-move direction can disarm" eight lines above the sentence
refuting it, and the review of that diff caught it (`R25`'s shape a second time).** Both mutants collapse
configurations, and deleting the read is the WIDER collapse (every substituted value, 0.15 included). What
decides the outcome is the check's form: a check asserting the *difference* between two collapsed arms is
disarmed (`:3255`, under both), and one making an *absolute* assertion about a collapsed arm reds when the
collapsed configuration violates it. ⚠️ What survives of direction is narrower: deleting the read leaves the
`nil` arm's input identical, which makes those greens **not evidence** rather than unable to fail in general.
✅ **And the seam's one constant-move-proof ARMED check is identified**: `:2634` substitutes **0.15**, a
...
have a hand-appended row, so the missing loadavg is this session's omission and not a limit. ✅ **The
`override-ignored` run appended one (`mutant-c26-inkbar-override 598 0 3.52`) — but its loadavg is NOT the
...
1,361 — 1.003x.** Three pairs now agree; ⛔ **the bound stays 1.043x, because an upper bound over several
readings is the LARGEST of them.**
...
the guard's FIRST term, is silent about the second, and is blind to the third. ⛔ **`shapeMinimumArea` STAYS UNPINNED ONLY IN THE LOWERING DIRECTION AS OF 2026-09-11** —
the RAISING one has a mutant and a fixture (`BUGS.md` C28 `#### The area floor's own fixture`), and
the bare mutant `SURVIVED` against the suite as it stood, which is what says this paragraph was
right that nothing pinned it. Kept, on a **narrower** argument than a first draft of this gave: at its shipped value, in
...
unmeasured and this entry's own Otsu mechanism predicts not.
✅ **AND ANOTHER MEMBER OF THE §4b ATTRIBUTION CLASS IS READ 2026-09-21 AND IT HOLDS —
`Jones et al_2010` p12's `1029x10+106+206` IS the table rule it is named after, and the `min` the box
distrusted is carried by 1,026 pixels rather than one** (`BUGS.md` C28 `#### The same loss at 1/3 …`'s
`##### The table rule's own rect, READ PER ROW AND PER COLUMN`; nothing in `Tools/` or `Sources/`
moved, no artefact written). ⛔ **No ordinal, and a draft carried two: *"the third member"* and *"the
first to hold"* are both refuted by the queue's own ticked boxes — the 59 px HELD and the drawing
rects' claims stood, so of the four answered members exactly ONE inverted.** The band's ten source
rows mean
`255 / 255 / 255 / 255 / 255 / 152.2 / 25.5 / 40.5 / 168.2 / 255` — one object, five clear rows above
it — and on the darkest row, **y=212**, min 25 = median 25 = p90 25 with **1,026 of 1,029 columns**
within 10 levels of the min in **one contiguous run, x 107…1132**; the three that are not are the
rect's own overhang. Read at 2x it is the thick middle rule of three in the paper's regression table.
✅ **The published 25 / 56 / 136 reproduces DIGIT FOR DIGIT and the control is sharper than a re-run**:
8-bit `25 / 57 / 136`, and unquantised `%[fx:minima*255]` reads **56.9819** and **136.45**, byte for
byte the full readings that section's own review item 8 recorded thirty-one days earlier (56 is it
truncated, 57 it rounded). ⛔ **Only ONE (filter, spec) pair yields 56.9819, so that agreement PINS
the 2026-08-21 pipeline rather than merely agreeing with it** — `default`/`Mitchell` 56.9819, Triangle
60.1759, Lanczos 35.0668, Point 24.0000, and the default at `-resize 200%` 63.8803. ⛔ **The instrument problem is real and it is a DIFFERENT one: the 1/2 figure
swings 24–60 over five ImageMagick resize filters and reads 57 against 64 between `-resize 1240x1647!`
and `-resize 200%`**, a one-row phase shift because 620x823 doubles to 1646 against a 1647-row source;
1/3 reads 118–136 and is spec-invariant. ⛔ **And the entry's own instrument-facts bullet names the
wrong filter: the bare `-resize` default is `Mitchell` — sha256-identical over the whole upsample —
not the Lanczos its *"kinder of the two"* argument rests on (default 57, Triangle 60, Lanczos 35, so
it is barely kinder than Triangle rather than notably so).** ⚠️ `Xin Qu` p20's verdict does NOT move
with it: that page was not measured here and its verdict rests on a 1:1 crop and a stencil ink of
0.0000. ✅ **The conclusion survives and the reason is measured rather
than argued: 1/3 keeps 60.2% of 1/2's contrast by band-min and 60.5% by darkest-row mean, and those two
agreeing to 0.3 points is what says the `min` is representative** (paper from the band's own y=206,
255.0 / 254.7 / 255.0, so the register's "~250" is pessimistic). ✅ *"Continuous at both factors"*
becomes a number — **1,028 of 1,029 columns below 200, longest run 1,028, at every factor.**
⛔ **The one clause that falls is *"thinner"*, and it is measured false in the frame the bullet is
written in — UPSAMPLED, where the rule is WIDER at 1/3 at every mean threshold it has rows at**: over
the RECT, 250/240/220/200/180 give `1/2` **7 / 6 / 4 / 4 / 4** against `1/3` **8 / 7 / 7 / 6 / 5**;
over a 30-row window y=196-225 — wider on purpose, since the falloff already leaves the rect at both
downsampled factors — **8 / 6 / 4 / 4 / 4** against **10 / 8 / 7 / 6 / 5**. Same sign at all five
either way. ⛔ **Both are printed because a draft published only the WINDOW pair under the rect's
name — three of its ten numerals were not readings of the rect, this step's own class inside the
step that exists to catch it, found by the adversarial review of the diff.** ⛔ **The
STORED frame must NOT be quoted and the first draft of this paragraph quoted it, at the one threshold
in it that favoured the conclusion** — raw stored rows read 3-against-2 at 250–220 (which reads as
*thinner*) and 1-against-2 at 200–180, and normalising for pixel pitch (a 1/2 row spans 2 source rows,
a 1/3 row spans 3) gives 6-against-6 then 2-against-6. One frame answers; the other is confounded
twice. ✅ *"Greyer"* is right and sharper — rows under a mean of 100 go **2 → 2 → 0** upsampled and
**2 → 1 → 0** stored, ⚠️ two frames and two sequences printed together for the reason above, so on
either reading no row of the rule reaches a mean of 100 at 1/3.
⛔ The mechanism is **PHASE**, not ink: the rule's ~4 source rows concentrate into one
stored row at 2x and straddle two at 3x, so the same ink is split and neither goes dark.
⛔ **And the step's sibling sweep over every *"darkest pixel"* claim in the tree found a member the
§4b sweep MISSED — `Riesman - 1954` p16's `900x150+300+2440`, where a darkest pixel over 135,000 px
carries *"peak contrast falls about 12.6%"*** — not called wrong, not measured, but there the `min`
...
paper at every factor, and at 1/3 the first inked row is 208.** Invisible to a `min` either way.
⛔ **The direction the queue box sent the
...
./run_tests.sh        # 1,374 checks, no skips. ⛔ THIS LINE MUST STAY UNDATED AND MUST CARRY THE
                      ...
                      #  1,371 -> 1,374: c28-area-fixture's too-small four — the AREA floor's
                      #  refusal, that page's own calibration reading, and the ink band-plus-
                      #  ratio. Same shape as the line above and the same free inverse row.
                      #  Measured off `mutate.py`'s own baseline (`1374 checks, green`) and the
                      #  mutant's `1373/1374`; the BARE mutant run before the fixture existed
                      #  printed `1371/1371`, which is where the 1,371 above is corroborated.)
