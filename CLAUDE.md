...
- `BUGS.md` — the defect register (2 MB). Read one entry with `ops/autonomous/bugs-entry.sh <TAG>`;
  never read the file whole. Open entries: C32, C33, C34 and C35 (found in 1.14.0 on 2026-09-25, queued first; C33 mostly fixed) and C28 (back in the queue
...
./run_tests.sh        # 1,447 checks, no skips
```
