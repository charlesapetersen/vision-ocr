...
| `score-threshold-loss.swift` | How much a page would lose to the 1-bit threshold. **Carries the four luminance signals R56 refused**, with a self-test that runs on every invocation. R56 is now `FIXED` by a shape signal instead (`Flattener.paleDrawing`), so this is history rather than a starting point — keep it for the four rounds' worth of reasons a luminance rule cannot work, and read `score-routing-census.swift` for what replaced it. |
